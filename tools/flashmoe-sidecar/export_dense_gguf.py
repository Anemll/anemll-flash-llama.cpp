...
MINIMAX_REASONING_HISTORY_SENTINEL = (
    "{%- if reasoning_content and loop.index0 > ns.last_user_index -%}"
)
MINIMAX_REASONING_HISTORY_GUARDED = (
    "{%- if reasoning_content and loop.index0 > ns.last_user_index and "
    "(enable_thinking is not defined or enable_thinking) -%}"
)
MINIMAX_GENERATION_PROMPT_SENTINEL = (
    "{%- if add_generation_prompt -%}\n"
    "{{- ']~b]ai' ~ '\\n' ~ '<think>' ~ '\\n' }}\n"
    "{%- endif -%}"
)
MINIMAX_GENERATION_PROMPT_GUARDED = (
    "{%- if add_generation_prompt -%}\n"
    "{{- ']~b]ai' ~ '\\n' }}\n"
...
def patch_minimax_chat_template(value: Any) -> Any:
    if isinstance(value, str):
        patched = value.replace(
            MINIMAX_REASONING_HISTORY_SENTINEL,
            MINIMAX_REASONING_HISTORY_GUARDED,
            1,
        )
    ...
    if isinstance(value, list):
        ...
        changed = False
        for item in value:
            patched_item = patch_minimax_chat_template(item)
            changed = changed or patched_item != item
            patched_list.append(patched_item)
        return patched_list if changed else value
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            patched_item = patch_minimax_chat_template(item)
            changed = changed or patched_item != item
            patched_dict[key] = patched_item
        return patched_dict if changed else value
...
def copy_metadata(reader: GGUFReader, writer: GGUFWriter) -> None:
    arch = str(reader_scalar(reader, gguf.Keys.General.ARCHITECTURE, "unknown"))
    for field in reader.fields.values():
        ...
        if arch == "minimax-m2" and field.name == gguf.Keys.Tokenizer.CHAT_TEMPLATE:
            value = patch_minimax_chat_template(value)
