...
### M5 Max Apple SSD vs external T710 sidecar

Controlled MiniMax `UD-Q4_K_XL-Flash` sidecar runs on `2026-04-14` used:

- `MODEL_PATH=~/Models/MiniMax-M2.7-GGUF/UD-Q4_K_XL-Flash/model-dense.gguf`
- `MOE_TOPK=4`
- `MOE_SLOT_BANK=60`
- `SEED=123`
- `TEMP=0.2`
- `-st -n 200 -p "Make a game of Space Invaders in pygame"`

To isolate the storage path, keep `MODEL_PATH` on the Apple SSD in both runs and switch only `SIDECAR_PATH`.

Observed takeaway from the controlled 3-run A/B:

- the external `T710` was parity-class with the internal Apple SSD on steady-state median routed source time
- the Apple path was a little steadier run-to-run
- practical conclusion: `T710` is good enough as either a primary sidecar lane or a secondary lane in `Apple + T710 + T710` multi-SSD experiments
