...
static inline float f8_e4m3fn_to_fp32(uint8_t q) {
    const uint8_t aq = q & 0x7F;

    if (aq == 0x7F) {
        return NAN;
    }

    const uint8_t e = aq >> 3;
    const uint8_t m = aq & 0x07;
    const float mag = float(e == 0 ? m : (8 + m)) * kvalues_f8_e4m3fn_scale_f[e];

    return (q & 0x80) ? -mag : mag;
}
...
void dequantize_f8_e4m3_b128(device const block_f8_e4m3_b128 * xb, short il, thread type4x4 & reg) {
    const float d = e8m0_to_fp32(xb->e);
    device const uint8_t * q = (device const uint8_t *)(xb->qs + 16*il);

    float4x4 reg_f;

    for (int i = 0; i < 16; ++i) {
        reg_f[i/4][i%4] = d * f8_e4m3fn_to_fp32(q[i]);
    }
...
void dequantize_f8_e4m3_b128_t4(device const block_f8_e4m3_b128 * xb, short il, thread type4 & reg) {
    const float d = e8m0_to_fp32(xb->e);
    device const uint8_t * q = (device const uint8_t *)(xb->qs + 4*il);

    for (int i = 0; i < 4; ++i) {
        reg[i] = d * f8_e4m3fn_to_fp32(q[i]);
    }
}
...
kernel void kernel_reglu_f16(
        ...
        uint tpitg[[thread_position_in_threadgroup]],
        uint   ntg[[threads_per_threadgroup]]) {
    device const half * src0_row = (device const half *) ((device const char *) src0 + tgpig*args.nb01) + args.i00;
    device const half * src1_row = (device const half *) ((device const char *) src1 + tgpig*args.nb11) + args.i10;
    device       half * dst_row  = (device       half *) ((device       char *) dst  + tgpig*args.nb1);

    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        const float x0 = (float) src0_row[i0];
        const float x1 = (float) src1_row[i0];

        dst_row[i0] = (half) (x0*x1*(x0 > 0.0f));
    }
}
...
kernel void kernel_swiglu_f16(
    ...
    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        ...
        dst_row[i0] = (half) (silu*x1);
    }
}

kernel void kernel_swiglu_limited_f32(
        constant ggml_metal_kargs_glu & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        uint tpitg[[thread_position_in_threadgroup]],
        uint   ntg[[threads_per_threadgroup]]) {
    device const float * src0_row = (device const float *) ((device const char *) src0 + tgpig*args.nb01);
    device const float * src1_row = (device const float *) ((device const char *) src1 + tgpig*args.nb11);
    device       float * dst_row  = (device       float *) ((device       char *) dst  + tgpig*args.nb1);

    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        const float x0 = min(src0_row[i0], args.limit);
...
kernel void kernel_swiglu_limited_f16(
        constant ggml_metal_kargs_glu & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        uint tpitg[[thread_position_in_threadgroup]],
        uint   ntg[[threads_per_threadgroup]]) {
    device const half * src0_row = (device const half *) ((device const char *) src0 + tgpig*args.nb01);
    device const half * src1_row = (device const half *) ((device const char *) src1 + tgpig*args.nb11);
    device       half * dst_row  = (device       half *) ((device       char *) dst  + tgpig*args.nb1);

    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        const float x0 = min((float) src0_row[i0], args.limit);
        const float x1 = clamp((float) src1_row[i0], -args.limit, args.limit);
        const float silu = x0 / (1.0f + exp(-x0));
...
kernel void kernel_swiglu_scaled_f32(
        constant ggml_metal_kargs_glu & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        uint   ntg[[threads_per_threadgroup]]) {
    device const float * src0_row = (device const float *) ((device const char *) src0 + tgpig*args.nb01);
    device const float * scale_row = (device const float *) ((device const char *) src1 + tgpig*args.nb11);
    device       float * dst_row   = (device       float *) ((device       char *) dst  + tgpig*args.nb1);

    const float scale = scale_row[0];

    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        const float x0 = src0_row[args.i00 + i0] * scale;
        const float x1 = src0_row[args.i10 + i0] * scale;
...
kernel void kernel_swiglu_oai_f16(
        constant ggml_metal_kargs_glu & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    for (int i0 = tpitg; i0 < args.ne0; i0 += ntg) {
        float x0 = (float) src0_row[i0];
        float x1 = (float) src1_row[i0];
...
[[host_name("kernel_mul_mv_shared_swiglu_q8_0_f32")]]
kernel void kernel_mul_mv_shared_swiglu_q8_0_f32(
        constant ggml_metal_kargs_mul_mv & args,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    src0_gate += args.src0_byte_off;
    src0_up   += args.src0_byte_off;
    src1      += args.src1_byte_off;
    dst       += args.dst_byte_off;
    ...
    device const block_q8_0 * gate_rows[NR0];
    device const block_q8_0 * up_rows[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        const uint64_t offset0 = (r0 + row)*args.nb01 + (i12/args.r2)*args.nb02 + (i13/args.r3)*args.nb03;
        gate_rows[row] = (device const block_q8_0 *) ((device const char *) src0_gate + offset0);
        up_rows[row]   = (device const block_q8_0 *) ((device const char *) src0_up   + offset0);
    }
    ...
    for (int ib = ib0; ib < nb; ib += NSG*NQ) {
        ...
        FOR_UNROLL (short row = 0; row < NR0; ++row) {
            device const int8_t * qg = gate_rows[row][ib].qs + il*NQ;
            device const int8_t * qu = up_rows[row][ib].qs + il*NQ;
            ...
            sum_gate[row] += sg*gate_rows[row][ib].d;
            sum_up[row]   += su*up_rows[row][ib].d;
        }
    ...
    FOR_UNROLL (short row = 0; row < NR0 && r0 + row < args.ne01; ++row) {
        ...
        if (tiisg == 0 && sgitg == 0) {
            dst_f32[r0 + row] = gate/(1.0f + exp(-gate))*up;
        }
    }
}

[[host_name("kernel_dsv4_routed_moe_shared_gate_up_swiglu_q8_0_f32")]]
kernel void kernel_dsv4_routed_moe_shared_gate_up_swiglu_q8_0_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0_gate,
        device const char * src0_up,
        device const char * src1,
    ...
    device float * dst_f32 = (device float *) dst + (uint64_t) im*args.ne0*args.ne1 + (uint64_t) r1*args.ne0;
    const uint64_t slot_stride = args.ne0;

    FOR_UNROLL (short row = 0; row < NR0 && r0 + row < args.ne01; ++row) {
        const float gate = simd_sum(sh_gate[row][tiisg]);
        const float up = simd_sum(sh_up[row][tiisg]);
        if (tiisg == 0 && sgitg == 0) {
            const float swiglu = gate/(1.0f + exp(-gate))*up;
            dst_f32[(uint64_t) 30 * slot_stride + r0 + row] = gate;
            dst_f32[(uint64_t) 31 * slot_stride + r0 + row] = up;
            dst_f32[(uint64_t) 32 * slot_stride + r0 + row] = swiglu;
        }
    }
}

[[host_name("kernel_dsv4_routed_moe_shared_down_final_q8_0_f32")]]
kernel void kernel_dsv4_routed_moe_shared_down_final_q8_0_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    threadgroup float * shmem_f32 = (threadgroup float *) shmem;
    threadgroup float * sh[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        sh[row] = shmem_f32 + NW*row;
        if (sgitg == 0) {
            sh[row][tiisg] = 0.0f;
        }
        sumf[row] = simd_sum(sumf[row]);
    ...
    FOR_UNROLL (short row = 0; row < NR0 && r0 + row < args.ne01; ++row) {
        const float shared = simd_sum(sh[row][tiisg]);
        if (tiisg == 0 && sgitg == 0) {
            const uint64_t i = r0 + row;
            dst_f32[(uint64_t) 33 * slot_stride + i] = shared;
            dst_f32[(uint64_t) 34 * slot_stride + i] = dst_f32[(uint64_t) 29 * slot_stride + i] + shared;
        }
    }
}
...
kernel void kernel_mul_mv_pair_f16_f32_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0_a,
        device const char * src0_b,
        device const char * src1,
    ...
    switch (args.nr0) {
        case 2:
            kernel_mul_mv_f16_f32_pair_4_impl<2, constant ggml_metal_kargs_mul_mv &>(
                    args, src0_a, src0_b, src1, dst_a, dst_b, shmem, tgpig, tiisg, sgitg);
            break;
        case 4:
            kernel_mul_mv_f16_f32_pair_4_impl<4, constant ggml_metal_kargs_mul_mv &>(
                    args, src0_a, src0_b, src1, dst_a, dst_b, shmem, tgpig, tiisg, sgitg);
            break;
    }
}
...
kernel void kernel_dsv4_hc_split_sinkhorn(
    ...
    device const float * mix        = (device const float *) (mixes + (uint64_t) tid*args.mixes_nb1);
    device const float * scale_data = (device const float *) scale;
    device const float * base_data  = (device const float *) base;
    device       float * out        = (device       float *) (dst + (uint64_t) tid*args.dst_nb1);

    const int n_hc = args.n_hc;
    ...
    for (int i = 0; i < n_hc; ++i) {
        ...
        out[i] = 1.0f / (1.0f + exp(-z)) + args.eps;
    }
    ...
    for (int src_hc = 0; src_hc < n_hc; ++src_hc) {
        ...
        const float inv_denom = 1.0f / (sum + args.eps);
        for (int dst_hc = 0; dst_hc < n_hc; ++dst_hc) {
...
static inline void dsv4_hc_split_sinkhorn_write_post_comb(
        constant ggml_metal_kargs_dsv4_hc_split_sinkhorn & args,
        device const float * mix,
        device const float * scale_data,
        device const float * base_data,
        device       float * out) {
    const int n_hc = args.n_hc;
    const float post_scale = scale_data[1];
    const float comb_scale = scale_data[2];
...
kernel void kernel_dsv4_hc_split_sinkhorn_weighted_sum(
        constant ggml_metal_kargs_dsv4_hc_split_sinkhorn & split_args,
        constant ggml_metal_kargs_dsv4_hc_weighted_sum & sum_args,
        device const char * mixes,
        device const char * scale,
        device const char * base,
        device const char * x,
        device       char * split_dst,
        device       char * sum_dst,
        ushort tiitg [[thread_index_in_threadgroup]],
        ushort3 tptg [[threads_per_threadgroup]],
        uint3 tgpig [[threadgroup_position_in_grid]]) {
    const int64_t chunk = tgpig.x;
    const int64_t t = tgpig.y;
    if (t >= split_args.n_rows || t >= sum_args.n_tokens) {
        return;
    }

    const int n_hc = split_args.n_hc;
    threadgroup float pre[16];

    if (tiitg == 0) {
        device const float * mix        = (device const float *) (mixes + (uint64_t) t*split_args.mixes_nb1);
        device const float * scale_data = (device const float *) scale;
        device const float * base_data  = (device const float *) base;
        ...
        for (int i = 0; i < n_hc; ++i) {
            const float z = mix[i] * pre_scale + base_data[i];
            const float v = 1.0f / (1.0f + exp(-z)) + split_args.eps;
            pre[i] = v;
        }

        if (chunk == 0) {
            device float * out = (device float *) (split_dst + (uint64_t) t*split_args.dst_nb1);

            for (int i = 0; i < n_hc; ++i) {
                out[i] = pre[i];
            }

            dsv4_hc_split_sinkhorn_write_post_comb(split_args, mix, scale_data, base_data, out);
        }
    }

    threadgroup_barrier(mem_flags::mem_threadgroup);

    const int64_t chunk_size = tptg.x;
    const int64_t d0 = chunk * chunk_size;
    const int64_t d1 = min(d0 + chunk_size, sum_args.n_embd);
    for (int64_t d = d0 + tiitg; d < d1; d += tptg.x) {
        float acc = 0.0f;
        for (int64_t h = 0; h < sum_args.n_hc; ++h) {
            const float xv = *(device const float *) (x + d*sum_args.x_nb0 + h*sum_args.x_nb1 + t*sum_args.x_nb2);
            acc += xv * pre[h];
        }

        *(device float *) (sum_dst + d*sum_args.dst_nb0 + t*sum_args.dst_nb1) = acc;
    }
}

kernel void kernel_dsv4_hc_split_sinkhorn_weighted_sum_norm(
        constant ggml_metal_kargs_dsv4_hc_split_sinkhorn & split_args,
        constant ggml_metal_kargs_dsv4_hc_weighted_sum & sum_args,
        constant ggml_metal_kargs_dsv4_hc_weighted_sum_norm & norm_args,
        device const char * mixes,
        device const char * scale,
        device const char * base,
        device const char * x,
    ...
    if ((int64_t) row >= split_args.n_rows ||
            (int64_t) row >= sum_args.n_tokens ||
            split_args.n_hc != 4 ||
            sum_args.n_hc != 4 ||
            norm_args.n_embd != sum_args.n_embd) {
        return;
    }

    threadgroup float4 * sum_cache = (threadgroup float4 *) shared;
    threadgroup float * pre = shared + norm_args.n_embd;
    threadgroup float * reduce = pre + 16;
    ...
    if (tid == 0) {
        device const float * mix        = (device const float *) (mixes + (uint64_t) row*split_args.mixes_nb1);
        device const float * scale_data = (device const float *) scale;
        device const float * base_data  = (device const float *) base;
        device       float * out        = (device       float *) (split_dst + (uint64_t) row*split_args.dst_nb1);
    ...
    float sumf = 0.0f;
    const int64_t n4 = norm_args.n_embd/4;

    for (int64_t i4 = tid; i4 < n4; i4 += ntg) {
        const int64_t d0 = 4*i4;
        float v0 = 0.0f;
        float v1 = 0.0f;
        float v2 = 0.0f;
        float v3 = 0.0f;
        for (int64_t h = 0; h < 4; ++h) {
            device const char * xh = x + h*sum_args.x_nb1 + (uint64_t) row*sum_args.x_nb2;
            v0 += *(device const float *) (xh + (d0 + 0)*sum_args.x_nb0) * pre[h];
            v1 += *(device const float *) (xh + (d0 + 1)*sum_args.x_nb0) * pre[h];
            v2 += *(device const float *) (xh + (d0 + 2)*sum_args.x_nb0) * pre[h];
            v3 += *(device const float *) (xh + (d0 + 3)*sum_args.x_nb0) * pre[h];
        }
        const float4 v = float4(v0, v1, v2, v3);
        sum_cache[i4] = v;
        sumf += dot(v, v);
    }
    ...
    sumf = reduce[tiisg];
    sumf = simd_sum(sumf);
    const float norm_scale = rsqrt(sumf / float(norm_args.n_embd) + norm_args.norm_eps);

    device float4 * sum4  = (device float4 *) (sum_dst  + (uint64_t) row*norm_args.sum_nb1);
    device float4 * mul4  = (device float4 *) (mul_dst  + (uint64_t) row*norm_args.mul_nb1);
    device const float4 * w4 = (device const float4 *) norm_weight;

    for (int64_t i4 = tid; i4 < n4; i4 += ntg) {
        const float4 v = sum_cache[i4];
        sum4[i4] = v;
        mul4[i4] = (v * norm_scale) * w4[i4];
    }
}
...
kernel void kernel_dsv4_hc_weighted_sum(
        ...
        device       char * dst,
        uint tid [[thread_position_in_grid]]) {
    if ((int64_t) tid >= args.n_elem) {
        return;
    }

    const int64_t d = tid % args.n_embd;
    const int64_t t = tid / args.n_embd;

    float acc = 0.0f;
    for (int64_t h = 0; h < args.n_hc; ++h) {
        const float xv = *(device const float *) (x       + d*args.x_nb0 + h*args.x_nb1 + t*args.x_nb2);
        const float wv = *(device const float *) (weights + h*args.w_nb0 + t*args.w_nb1);
        acc += xv * wv;
    }

    *(device float *) (dst + d*args.dst_nb0 + t*args.dst_nb1) = acc;
}
...
kernel void kernel_dsv4_hc_expand(
    ...
    const int64_t d      = tid % args.n_embd;
    const int64_t tmp    = tid / args.n_embd;
    const int64_t dst_hc = tmp % args.n_hc;
    const int64_t t      = tmp / args.n_hc;

    const float block_v = *(device const float *) (block_out + d*args.block_nb0 + t*args.block_nb1);
    const float post_v  = *(device const float *) (post      + dst_hc*args.post_nb0 + t*args.post_nb1);

    float acc = block_v * post_v;
    for (int64_t src_hc = 0; src_hc < args.n_hc; ++src_hc) {
        const float comb_v = *(device const float *) (comb     + dst_hc*args.comb_nb0 + src_hc*args.comb_nb1 + t*args.comb_nb2);
        const float res_v  = *(device const float *) (residual + d*args.res_nb0       + src_hc*args.res_nb1  + t*args.res_nb2);
        acc += comb_v * res_v;
    }

    *(device float *) (dst + d*args.dst_nb0 + dst_hc*args.dst_nb1 + t*args.dst_nb2) = acc;
}
...
kernel void kernel_dsv4_hc_expand4(
    ...
    const float r0 = *(device const float *) (residual + d*args.res_nb0 + 0*args.res_nb1 + t*args.res_nb2);
    const float r1 = *(device const float *) (residual + d*args.res_nb0 + 1*args.res_nb1 + t*args.res_nb2);
    const float r2 = *(device const float *) (residual + d*args.res_nb0 + 2*args.res_nb1 + t*args.res_nb2);
    const float r3 = *(device const float *) (residual + d*args.res_nb0 + 3*args.res_nb1 + t*args.res_nb2);

    for (int64_t dst_hc = 0; dst_hc < 4; ++dst_hc) {
        float acc = block_v * *(device const float *) (post + dst_hc*args.post_nb0 + t*args.post_nb1);

        acc += *(device const float *) (comb + dst_hc*args.comb_nb0 + 0*args.comb_nb1 + t*args.comb_nb2) * r0;
        acc += *(device const float *) (comb + dst_hc*args.comb_nb0 + 1*args.comb_nb1 + t*args.comb_nb2) * r1;
        acc += *(device const float *) (comb + dst_hc*args.comb_nb0 + 2*args.comb_nb1 + t*args.comb_nb2) * r2;
        acc += *(device const float *) (comb + dst_hc*args.comb_nb0 + 3*args.comb_nb1 + t*args.comb_nb2) * r3;

        *(device float *) (dst + d*args.dst_nb0 + dst_hc*args.dst_nb1 + t*args.dst_nb2) = acc;
    }
}

[[host_name("kernel_dsv4_q8_hc_expand_q8_0")]]
kernel void kernel_dsv4_q8_hc_expand_q8_0(
        constant ggml_metal_kargs_mul_mv        & mv,
        constant ggml_metal_kargs_dsv4_hc_expand & hc,
    ...
    const uint i12 = im%mv.ne12;
    const uint i13 = im/mv.ne12;

    const uint64_t offset1 = (uint64_t) r1*mv.nb11 + (uint64_t) i12*mv.nb12 + (uint64_t) i13*mv.nb13;
    device const float * y = (device const float *) (input + mv.src1_byte_off + offset1);

    device const block_q8_0 * ax[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        const uint64_t offset0 = (uint64_t) (r0 + row)*mv.nb01 + (uint64_t) (i12/mv.r2)*mv.nb02 + (uint64_t) (i13/mv.r3)*mv.nb03;
        ax[row] = (device const block_q8_0 *) (weight + mv.src0_byte_off + offset0);
    }
    ...
    float total[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        total[row] = simd_sum(shmem_f32[row][tiisg]);
    }
    ...
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        const int64_t d = r0 + row;
        if (d >= mv.ne01) {
            continue;
        }

        const float block_v = total[row];

        *(device float *) (block_out + d*hc.block_nb0 + t*hc.block_nb1) = block_v;

        for (int64_t dst_hc = 0; dst_hc < hc.n_hc; ++dst_hc) {
            const float post_v = *(device const float *) (post + dst_hc*hc.post_nb0 + t*hc.post_nb1);

            float acc = block_v * post_v;
            for (int64_t src_hc = 0; src_hc < hc.n_hc; ++src_hc) {
                const float comb_v = *(device const float *) (comb     + dst_hc*hc.comb_nb0 + src_hc*hc.comb_nb1 + t*hc.comb_nb2);
                const float res_v  = *(device const float *) (residual + d*hc.res_nb0       + src_hc*hc.res_nb1  + t*hc.res_nb2);
                acc += comb_v * res_v;
            }

            *(device float *) (dst + d*hc.dst_nb0 + dst_hc*hc.dst_nb1 + t*hc.dst_nb2) = acc;
        }
    }
}

static inline float dsv4_e4m3fn_quant_dequant(float x) {
    ...
    for (int i = 1; i < 127; ++i) {
        const float val = f8_e4m3fn_to_fp32((uint8_t) i);
    ...
    return sign * f8_e4m3fn_to_fp32((uint8_t) best);
}

kernel void kernel_dsv4_fp8_kv_quantize(
        constant ggml_metal_kargs_dsv4_fp8_kv_quantize & args,
        device const char * src0,
        device       char * dst,
        uint tid [[thread_position_in_grid]]) {
    const int64_t work_per_row = args.n_blocks + 1;
    const int64_t n_work = args.n_rows * work_per_row;
    if ((int64_t) tid >= n_work) {
        return;
    }

    const int64_t row = tid / work_per_row;
    const int64_t blk = tid - row*work_per_row;

    const int64_t i1 = row % args.ne1;
    const int64_t i2 = (row / args.ne1) % args.ne2;
    const int64_t i3 = row / (args.ne1 * args.ne2);

    device const char * src_base = src0 + i1*args.src_nb1 + i2*args.src_nb2 + i3*args.src_nb3;
    device       char * dst_base = dst  + i1*args.dst_nb1 + i2*args.dst_nb2 + i3*args.dst_nb3;

    if (blk < args.n_blocks) {
        const int64_t off = blk * 64;

        float amax = 0.0f;
        for (int64_t i = 0; i < 64; ++i) {
            const float v = *(device const float *) (src_base + (off + i)*args.src_nb0);
            amax = max(amax, abs(v));
        }
        ...
        for (int64_t i = 0; i < 64; ++i) {
            const float v = *(device const float *) (src_base + (off + i)*args.src_nb0);
            const float q = min(max(v / scale, -448.0f), 448.0f);
            *(device float *) (dst_base + (off + i)*args.dst_nb0) = dsv4_e4m3fn_quant_dequant(q) * scale;
        }
    } else {
        for (int64_t i = args.n_nope; i < args.head_dim; ++i) {
            *(device float *) (dst_base + i*args.dst_nb0) = *(device const float *) (src_base + i*args.src_nb0);
        }
    }
}

static inline float dsv4_fp4_quant_dequant(float x) {
    const float sign = x < 0.0f ? -1.0f : 1.0f;
    ...
    float best_diff = ax;
    for (int i = 1; i < 8; ++i) {
        const float diff = abs(ax - kvalues_mxfp4_f[i]);
        if (diff < best_diff || (diff == best_diff && (i & 1) == 0 && (best & 1) != 0)) {
            best = i;
            best_diff = diff;
        }
    ...
    return sign * kvalues_mxfp4_f[best];
}

kernel void kernel_dsv4_hadamard_fp4_quantize(
        constant ggml_metal_kargs_dsv4_hadamard_fp4_quantize & args,
        device const char * src0,
        device       char * dst,
        uint tid [[thread_position_in_grid]]) {
    if ((int64_t) tid >= args.n_rows) {
    ...
    const int64_t i1 = tid % args.ne1;
    const int64_t i2 = (tid / args.ne1) % args.ne2;
    const int64_t i3 = tid / (args.ne1 * args.ne2);

    device const char * src_base = src0 + i1*args.src_nb1 + i2*args.src_nb2 + i3*args.src_nb3;
    device       char * dst_base = dst  + i1*args.dst_nb1 + i2*args.dst_nb2 + i3*args.dst_nb3;

    float v[256];
    for (int64_t i = 0; i < args.head_dim; ++i) {
        v[i] = *(device const float *) (src_base + i*args.src_nb0);
    }

    for (int64_t step = 1; step < args.head_dim; step <<= 1) {
        for (int64_t base = 0; base < args.head_dim; base += 2*step) {
            for (int64_t i = 0; i < step; ++i) {
                const float a = v[base + i];
                const float b = v[base + i + step];
                v[base + i]        = a + b;
                v[base + i + step] = a - b;
            }
        }
    ...
    const float inv_sqrt_dim = rsqrt(float(args.head_dim));
    for (int64_t block = 0; block < args.head_dim; block += 32) {
        float amax = 0.0f;
        for (int64_t i = 0; i < 32; ++i) {
            v[block + i] *= inv_sqrt_dim;
            amax = max(amax, abs(v[block + i]));
        }

        amax = max(amax, 6.0f * exp2(-126.0f));
        const float scale = exp2(ceil(log2(amax / 6.0f)));
        for (int64_t i = 0; i < 32; ++i) {
            const float q = min(max(v[block + i] / scale, -6.0f), 6.0f);
            *(device float *) (dst_base + (block + i)*args.dst_nb0) = dsv4_fp4_quant_dequant(q) * scale;
        }
    }
}

kernel void kernel_dsv4_indexer_weighted_score(
        constant ggml_metal_kargs_dsv4_indexer_weighted_score & args,
    ...
    const int64_t c = tid % args.n_comp;
    const int64_t t = tid / args.n_comp;

    float acc = 0.0f;
    for (int64_t h = 0; h < args.n_heads; ++h) {
        const float sv = *(device const float *) (score   + c*args.score_nb0 + t*args.score_nb1 + h*args.score_nb2);
        const float wv = *(device const float *) (weights + h*args.weights_nb0 + t*args.weights_nb1);
        acc += max(sv, 0.0f) * wv;
    }

    *(device float *) (dst + c*args.dst_nb0 + t*args.dst_nb1) = acc * args.scale;
}

static inline void dsv4_mixed_attend_shared_half_row(
    ...
    score = simd_sum(score) * scale + bias;
...
static inline void dsv4_mixed_attend_shared_half_row_at(
        threadgroup const half4 * kv4,
        uint row,
        half4 q0,
        half4 q1,
        ...
        thread float4 & o2,
        thread float4 & o3) {
    dsv4_mixed_attend_shared_half_row(kv4 + row*128u, q0, q1, q2, q3, scale, bias, lane, M, S, o0, o1, o2, o3);
}

static inline void dsv4_mixed_attend_sink(
        float score,
        thread float & M,
        thread float & S,
        thread float4 & o0,
...
kernel void kernel_dsv4_mixed_attn_f16(
        constant ggml_metal_kargs_dsv4_mixed_attn & args,
        device const char * q,
    ...
    if (head >= (uint) args.n_heads) {
        return;
    }

    device const float4 * q4 = (device const float4 *) (q + (uint64_t) token * args.q_nb2 + (uint64_t) head * args.q_nb1);
    ...
    for (int row0 = 0; row0 < args.n_raw; row0 += 4) {
        uint rows[4];
        float biases[4];
        uint n_rows = 0;
        for (uint j = 0; j < 4u && row0 + int(j) < args.n_raw; ++j) {
            const int row = row0 + int(j);
            const float bias = *(device const float *) (raw_mask + (uint64_t) row * args.raw_mask_nb0 + (uint64_t) token * args.raw_mask_nb1);
            if (!isfinite(bias)) {
                continue;
            }
            rows[n_rows] = (uint) row;
            biases[n_rows] = bias;
            ++n_rows;
        }
        if (n_rows == 0) {
        ...
        for (uint off = (uint) tid; off < n_rows*128u; off += 256u) {
            ...
            device const half4 * src = (device const half4 *) (raw_kv + (uint64_t) rows[r] * args.raw_nb2);
            kv_shared[off] = src[c];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint r = 0; r < n_rows; ++r) {
            dsv4_mixed_attend_shared_half_row_at(kv_shared, r, q0, q1, q2, q3, args.scale, biases[r], lane, M, S, o0, o1, o2, o3);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    for (int row0 = 0; row0 < args.n_comp; row0 += 4) {
        uint rows[4];
        float biases[4];
        uint n_rows = 0;
        for (uint j = 0; j < 4u && row0 + int(j) < args.n_comp; ++j) {
            const int row = row0 + int(j);
            const float bias = *(device const float *) (comp_mask + (uint64_t) row * args.comp_mask_nb0 + (uint64_t) token * args.comp_mask_nb1);
            if (!isfinite(bias)) {
                continue;
            }
            rows[n_rows] = (uint) row;
        ...
        for (uint off = (uint) tid; off < n_rows*128u; off += 256u) {
            const uint r = off >> 7;
            const uint c = off & 127u;
            device const half4 * src = (device const half4 *) (comp_kv + (uint64_t) rows[r] * args.comp_nb2);
            kv_shared[off] = src[c];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint r = 0; r < n_rows; ++r) {
    ...
    dsv4_mixed_attend_sink(((device const float *) sinks)[head], M, S, o0, o1, o2, o3);

    const float inv_s = S == 0.0f ? 0.0f : 1.0f/S;
    device float4 * dst4 = (device float4 *) (dst + (uint64_t) token * args.dst_nb2 + (uint64_t) head * args.dst_nb1);
...
static inline float dsv4_rope_tail_read(
        constant ggml_metal_kargs_dsv4_rope_tail & args,
        device const char * src0,
        device const char * src1,
        device const char * src2,
        thread float * corr_dims,
        int64_t i0,
        int64_t i1,
        int64_t i2,
        int64_t i3) {
    device const T * const src = (device const T *) (src0 + i3*args.nb03 + i2*args.nb02 + i1*args.nb01);

    if (i0 < args.n_nope) {
        return (float) src[i0];
    }

    const int64_t tail = i0 - args.n_nope;
    int ic;
    bool first;
    int off0;
    int off1;
    ...
    const float sin_sign = args.inverse ? -1.0f : 1.0f;
    const int rope_i0 = 2*ic;
    const float theta = theta_base * pow(args.freq_base, inv_ndims*rope_i0);
    const float freq_factor = args.src2 ? ((device const float *) src2)[ic] : 1.0f;

    float cos_theta;
    float sin_theta;
    rope_yarn(theta/freq_factor, args.freq_scale, corr_dims, rope_i0, args.ext_factor, args.attn_factor, &cos_theta, &sin_theta);
    sin_theta *= sin_sign;

    const float x0 = (float) src[off0];
    const float x1 = (float) src[off1];

    return first ? (x0*cos_theta - x1*sin_theta) : (x0*sin_theta + x1*cos_theta);
}

template<bool neox>
kernel void kernel_dsv4_decode_compress(
        constant ggml_metal_kargs_dsv4_decode_compress & args,
        device const char * kv,
        device const char * score,
        device const char * norm,
        device const char * pos_buf,
        device       char * dst,
        ushort tiitg [[thread_index_in_threadgroup]],
        ushort3 tptg [[threads_per_threadgroup]]) {
    threadgroup float v[1024];
    threadgroup float ss_parts[256];
    threadgroup float inv_rms;

    float ss = 0.0f;
    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        float max_score = -INFINITY;
        for (int64_t j = 0; j < args.n_pool; ++j) {
            const float s = *(device const float *) (score + i*args.score_nb0 + j*args.score_nb1);
        ...
        for (int64_t j = 0; j < args.n_pool; ++j) {
            const float s = *(device const float *) (score + i*args.score_nb0 + j*args.score_nb1);
            const float w = exp(s - max_score);
            const float x = *(device const float *) (kv + i*args.kv_nb0 + j*args.kv_nb1);
            denom += w;
            acc += x*w;
        }
        ...
        v[i] = pooled;
        if (args.output_stage == 1) {
            *(device float *) (dst + i*args.dst_nb0) = pooled;
        }
        ss += pooled*pooled;
    }
    ...
    for (uint stride = tptg.x >> 1; stride > 0; stride >>= 1) {
        if (tiitg < stride) {
            ss_parts[tiitg] += ss_parts[tiitg + stride];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    if (tiitg == 0) {
        inv_rms = rsqrt(ss_parts[0] / float(args.head_dim) + args.norm_eps);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        const float nw = *(device const float *) (norm + i*args.norm_nb0);
        const float normed = v[i] * inv_rms;
        if (args.output_stage == 2) {
            *(device float *) (dst + i*args.dst_nb0) = normed;
        }
        const float weighted = normed * nw;
        if (args.output_stage == 3 || args.output_stage == 4) {
            *(device float *) (dst + i*args.dst_nb0) = weighted;
        }
        v[i] = weighted;
    }

    if (args.output_stage == 2 || args.output_stage == 3 || args.output_stage == 4) {
        return;
    }
    ...
    device const int32_t * pos = (device const int32_t *) pos_buf;
    ...
    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        float out = v[i];
        if (i >= args.n_nope) {
            const int64_t tail = i - args.n_nope;
            int ic;
            bool first;
            int off0;
            int off1;
            ...
            float cos_theta;
            float sin_theta;
            rope_yarn(theta, args.freq_scale, corr_dims, rope_i0, args.ext_factor, args.attn_factor, &cos_theta, &sin_theta);
        ...
        *(device float *) (dst + i*args.dst_nb0) = out;
    }
}

typedef decltype(kernel_dsv4_decode_compress<false>) kernel_dsv4_decode_compress_t;

template [[host_name("kernel_dsv4_decode_compress_norm")]] kernel kernel_dsv4_decode_compress_t kernel_dsv4_decode_compress<false>;
template [[host_name("kernel_dsv4_decode_compress_neox")]] kernel kernel_dsv4_decode_compress_t kernel_dsv4_decode_compress<true>;

template<bool neox>
kernel void kernel_dsv4_compressor_update_decode(
        constant ggml_metal_kargs_dsv4_compressor_update_decode & args,
        device const char * pair_kv,
        device const char * pair_score,
        device const char * prev_kv,
        device const char * prev_score,
        ...
        ushort3 tptg [[threads_per_threadgroup]],
        ushort3 tgpig [[threadgroup_position_in_grid]],
        ushort3 tgpg [[threadgroups_per_grid]]) {
    threadgroup float v[1024];
    threadgroup float ss_parts[256];
    threadgroup float inv_rms;

    device char * dst_kv    = dst;
    device char * dst_score = dst + args.state_elems*sizeof(float);
    device char * dst_comp  = dst + 2*args.state_elems*sizeof(float);
    device char * dst_pool_kv = dst + (2*args.state_elems + args.head_dim + 2*args.width)*sizeof(float);
    device char * dst_pool_score = dst_pool_kv + args.head_dim*args.n_pool*sizeof(float);

    const int64_t state_tid = int64_t(tgpig.x)*int64_t(tptg.x) + int64_t(tiitg);
    const int64_t state_stride = int64_t(tgpg.x)*int64_t(tptg.x);

    for (int64_t idx = state_tid; idx < args.state_elems; idx += state_stride) {
        const int64_t col = idx % args.width;
        const int64_t out_row = idx / args.width;
        int64_t src_row = out_row;
        if (args.should_compress && args.compress_ratio == 4) {
            src_row = args.compress_ratio + (out_row % args.compress_ratio);
        }

        float kv;
        float sc;
        if (src_row == args.row) {
            kv = *(device const float *) (pair_kv + col*sizeof(float));
            sc = *(device const float *) (pair_score + col*sizeof(float)) +
                    *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
        } else {
            kv = *(device const float *) (prev_kv + col*args.prev_kv_nb0 + src_row*args.prev_kv_nb1);
            sc = *(device const float *) (prev_score + col*args.prev_score_nb0 + src_row*args.prev_score_nb1);
        }

        *(device float *) (dst_kv + idx*sizeof(float)) = kv;
        *(device float *) (dst_score + idx*sizeof(float)) = sc;
    }

    if (args.should_compress && !args.fused_comp) {
        const int64_t pool_elems = args.head_dim * args.n_pool;
        for (int64_t idx = state_tid; idx < pool_elems; idx += state_stride) {
            const int64_t i = idx % args.head_dim;
            const int64_t j = idx / args.head_dim;
            const int64_t col = args.compress_ratio == 4 && j >= args.compress_ratio ? args.head_dim + i : i;
            const int64_t row = j;
            ...
            if (row == args.row) {
                ...
                kv = *(device const float *) (prev_kv + col*args.prev_kv_nb0 + row*args.prev_kv_nb1);
                sc = *(device const float *) (prev_score + col*args.prev_score_nb0 + row*args.prev_score_nb1);
            }

            *(device float *) (dst_pool_kv + idx*sizeof(float)) = kv;
            *(device float *) (dst_pool_score + idx*sizeof(float)) = sc;
        }
    }
    ...
    if (!args.fused_comp) {
        for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
            *(device float *) (dst_comp + i*args.dst_nb0) = 0.0f;
        }
        return;
    }
    ...
    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        ...
        for (int64_t j = 0; j < args.n_pool; ++j) {
            const int64_t col = args.compress_ratio == 4 && j >= args.compress_ratio ? args.head_dim + i : i;
            const int64_t row = args.compress_ratio == 4 && j >= args.compress_ratio ? j : j;
            float s;
            if (row == args.row) {
                s = *(device const float *) (pair_score + col*sizeof(float)) +
                        *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
            } else {
                s = *(device const float *) (prev_score + col*args.prev_score_nb0 + row*args.prev_score_nb1);
            }
            max_score = max(max_score, s);
        }
        ...
        for (int64_t j = 0; j < args.n_pool; ++j) {
            ...
            if (row == args.row) {
                s = *(device const float *) (pair_score + col*sizeof(float)) +
                        *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
                x = *(device const float *) (pair_kv + col*sizeof(float));
            } else {
                s = *(device const float *) (prev_score + col*args.prev_score_nb0 + row*args.prev_score_nb1);
                x = *(device const float *) (prev_kv + col*args.prev_kv_nb0 + row*args.prev_kv_nb1);
            }
            const float w = exp(s - max_score);
            denom += w;
            acc += x*w;
    ...
    const float theta_base = float(args.pos + 1 - args.compress_ratio);
    const float inv_ndims = -1.0f / args.n_dims;
    ...
    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        ...
        *(device float *) (dst_comp + i*args.dst_nb0) = out;
    }
}

typedef decltype(kernel_dsv4_compressor_update_decode<false>) kernel_dsv4_compressor_update_decode_t;

template [[host_name("kernel_dsv4_compressor_update_decode_norm")]] kernel kernel_dsv4_compressor_update_decode_t kernel_dsv4_compressor_update_decode<false>;
template [[host_name("kernel_dsv4_compressor_update_decode_neox")]] kernel kernel_dsv4_compressor_update_decode_t kernel_dsv4_compressor_update_decode<true>;

template<bool neox>
kernel void kernel_dsv4_compressor_update_decode_v2(
        constant ggml_metal_kargs_dsv4_compressor_update_decode_v2 & args,
        device const char * pair_kv,
        device const char * pair_score,
        device const char * prev_kv,
        device const char * prev_score,
    ...
    device char * dst_score = dst + args.state_elems*sizeof(float);
    device char * dst_comp  = dst + 2*args.state_elems*sizeof(float);
    device char * dst_pool_kv = dst + args.pool_off_elems*sizeof(float);
    device char * dst_pool_score = dst_pool_kv + args.head_dim*args.n_pool*sizeof(float);
    ...
    for (int64_t idx = state_tid; idx < args.state_elems; idx += state_stride) {
        ...
        float sc;
        if (src_row == args.row) {
            kv = *(device const float *) (pair_kv + col*args.kv_nb0);
            sc = *(device const float *) (pair_score + col*args.score_nb0) +
                    *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
        } else {
            kv = *(device const float *) (prev_kv + col*args.prev_kv_nb0 + src_row*args.prev_kv_nb1);
            sc = *(device const float *) (prev_score + col*args.prev_score_nb0 + src_row*args.prev_score_nb1);
    ...
    for (int64_t i = tiitg; i < args.head_dim; i += tptg.x) {
        ...
        for (int64_t j = 0; j < args.n_pool; ++j) {
            ...
            float s;
            if (row == args.row) {
                s = *(device const float *) (pair_score + col*args.score_nb0) +
                        *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
            } else {
                s = *(device const float *) (prev_score + col*args.prev_score_nb0 + row*args.prev_score_nb1);
            }
        ...
        for (int64_t j = 0; j < args.n_pool; ++j) {
            ...
            if (row == args.row) {
                s = *(device const float *) (pair_score + col*args.score_nb0) +
                        *(device const float *) (ape + col*args.ape_nb0 + args.pos_mod*args.ape_nb1);
                x = *(device const float *) (pair_kv + col*args.kv_nb0);
            } else {
                s = *(device const float *) (prev_score + col*args.prev_score_nb0 + row*args.prev_score_nb1);
                x = *(device const float *) (prev_kv + col*args.prev_kv_nb0 + row*args.prev_kv_nb1);
            }
...
typedef decltype(kernel_dsv4_compressor_update_decode_v2<false>) kernel_dsv4_compressor_update_decode_v2_t;

template [[host_name("kernel_dsv4_compressor_update_decode_v2_norm")]] kernel kernel_dsv4_compressor_update_decode_v2_t kernel_dsv4_compressor_update_decode_v2<false>;
template [[host_name("kernel_dsv4_compressor_update_decode_v2_neox")]] kernel kernel_dsv4_compressor_update_decode_v2_t kernel_dsv4_compressor_update_decode_v2<true>;

template<typename T, bool neox>
kernel void kernel_dsv4_rope_hadamard_fp4_quantize(
        constant ggml_metal_kargs_dsv4_rope_tail & rope_args,
        constant ggml_metal_kargs_dsv4_hadamard_fp4_quantize & quant_args,
        device const char * src0,
        device const char * src1,
        device const char * src2,
        device       char * dst,
        uint tid [[thread_position_in_grid]]) {
    if ((int64_t) tid >= quant_args.n_rows) {
        return;
    }

    const int64_t i1 = tid % quant_args.ne1;
    const int64_t i2 = (tid / quant_args.ne1) % quant_args.ne2;
    const int64_t i3 = tid / (quant_args.ne1 * quant_args.ne2);

    device char * dst_base = dst + i1*quant_args.dst_nb1 + i2*quant_args.dst_nb2 + i3*quant_args.dst_nb3;

    float corr_dims[2];
    rope_yarn_corr_dims(rope_args.n_dims, rope_args.n_ctx_orig, rope_args.freq_base, rope_args.beta_fast, rope_args.beta_slow, corr_dims);

    float v[256];
    for (int64_t i = 0; i < quant_args.head_dim; ++i) {
        v[i] = dsv4_rope_tail_read<T, neox>(rope_args, src0, src1, src2, corr_dims, i, i1, i2, i3);
    }

    for (int64_t step = 1; step < quant_args.head_dim; step <<= 1) {
        for (int64_t base = 0; base < quant_args.head_dim; base += 2*step) {
            for (int64_t i = 0; i < step; ++i) {
                const float a = v[base + i];
                const float b = v[base + i + step];
                v[base + i]        = a + b;
    ...
    const float inv_sqrt_dim = rsqrt(float(quant_args.head_dim));
    for (int64_t block = 0; block < quant_args.head_dim; block += 32) {
        float amax = 0.0f;
        for (int64_t i = 0; i < 32; ++i) {
            v[block + i] *= inv_sqrt_dim;
            amax = max(amax, abs(v[block + i]));
        ...
        for (int64_t i = 0; i < 32; ++i) {
            const float q = min(max(v[block + i] / scale, -6.0f), 6.0f);
            *(device float *) (dst_base + (block + i)*quant_args.dst_nb0) = dsv4_fp4_quant_dequant(q) * scale;
        }
    }
}

template<typename T, bool neox>
kernel void kernel_dsv4_rope_fp8_kv_quantize(
        constant ggml_metal_kargs_dsv4_rope_tail & rope_args,
        constant ggml_metal_kargs_dsv4_fp8_kv_quantize & quant_args,
        device const char * src0,
        device const char * src1,
        device const char * src2,
        device       char * dst,
    ...
    if (row >= quant_args.n_rows) {
        return;
    }

    const int64_t i1 = row % quant_args.ne1;
    const int64_t i2 = (row / quant_args.ne1) % quant_args.ne2;
    const int64_t i3 = row / (quant_args.ne1 * quant_args.ne2);

    device const T * const src = (device const T *) (src0 + i3*rope_args.nb03 + i2*rope_args.nb02 + i1*rope_args.nb01);
    device       char * dst_base = dst + i1*quant_args.dst_nb1 + i2*quant_args.dst_nb2 + i3*quant_args.dst_nb3;
    ...
    threadgroup float v[1024];
    for (int64_t i = tiitg; i < quant_args.n_nope; i += tptg.x) {
        v[i] = (float) src[i];
    }
    for (int64_t i = quant_args.n_nope + tiitg; i < quant_args.head_dim; i += tptg.x) {
        v[i] = dsv4_rope_tail_read<T, neox>(rope_args, src0, src1, src2, corr_dims, i, i1, i2, i3);
    }

    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (int64_t blk = tiitg; blk < quant_args.n_blocks; blk += tptg.x) {
        const int64_t off = blk * 64;

        float amax = 0.0f;
        for (int64_t i = 0; i < 64; ++i) {
            amax = max(amax, abs(v[off + i]));
        }

        amax = max(amax, 1.0e-4f);
        const float scale = exp2(ceil(log2(amax / 448.0f)));
        for (int64_t i = 0; i < 64; ++i) {
            const float q = min(max(v[off + i] / scale, -448.0f), 448.0f);
            *(device float *) (dst_base + (off + i)*quant_args.dst_nb0) = dsv4_e4m3fn_quant_dequant(q) * scale;
        }
    }

    for (int64_t i = quant_args.n_nope + tiitg; i < quant_args.head_dim; i += tptg.x) {
        *(device float *) (dst_base + i*quant_args.dst_nb0) = v[i];
    }
}

template<typename T, typename TC, typename TI, bool neox>
kernel void kernel_dsv4_rope_fp8_kv_set_rows(
        constant ggml_metal_kargs_dsv4_rope_tail & rope_args,
        constant ggml_metal_kargs_dsv4_fp8_kv_quantize & quant_args,
        constant ggml_metal_kargs_set_rows & set_args,
        device const char * src0,
        device const char * src1,
        device const char * src2,
        device       char * dst,
        device const char * set_idxs,
        device       char * set_dst,
        ushort tiitg [[thread_index_in_threadgroup]],
        ushort3 tptg [[threads_per_threadgroup]],
        uint3 tgpig [[threadgroup_position_in_grid]]) {
    ...
    const TI set_row = ((device const TI *) (set_idxs + row*set_args.nb10))[0];
    device char * set_base = set_dst + (int64_t)set_row*set_args.nb1;
    ...
    for (int64_t blk = tiitg; blk < quant_args.n_blocks; blk += tptg.x) {
        ...
        for (int64_t i = 0; i < 64; ++i) {
            const float q = min(max(v[off + i] / scale, -448.0f), 448.0f);
            const float dq = dsv4_e4m3fn_quant_dequant(q) * scale;
            *(device float *) (dst_base + (off + i)*quant_args.dst_nb0) = dq;
            *(device TC *) (set_base + (off + i)*sizeof(TC)) = (TC) dq;
        }
    }

    for (int64_t i = quant_args.n_nope + tiitg; i < quant_args.head_dim; i += tptg.x) {
        *(device float *) (dst_base + i*quant_args.dst_nb0) = v[i];
        *(device TC *) (set_base + i*sizeof(TC)) = (TC) v[i];
    }
}

typedef decltype(kernel_dsv4_rope_hadamard_fp4_quantize<float, false>) kernel_dsv4_rope_hadamard_fp4_quantize_t;

template [[host_name("kernel_dsv4_rope_hadamard_fp4_quantize_norm_f32")]] kernel kernel_dsv4_rope_hadamard_fp4_quantize_t kernel_dsv4_rope_hadamard_fp4_quantize<float, false>;
template [[host_name("kernel_dsv4_rope_hadamard_fp4_quantize_neox_f32")]] kernel kernel_dsv4_rope_hadamard_fp4_quantize_t kernel_dsv4_rope_hadamard_fp4_quantize<float, true>;

typedef decltype(kernel_dsv4_rope_fp8_kv_quantize<float, false>) kernel_dsv4_rope_fp8_kv_quantize_t;
typedef decltype(kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int32_t, false>) kernel_dsv4_rope_fp8_kv_set_rows_f16_i32_t;
typedef decltype(kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int64_t, false>) kernel_dsv4_rope_fp8_kv_set_rows_f16_i64_t;
typedef decltype(kernel_dsv4_rope_fp8_kv_set_rows<float, float, int32_t, false>) kernel_dsv4_rope_fp8_kv_set_rows_f32_i32_t;
typedef decltype(kernel_dsv4_rope_fp8_kv_set_rows<float, float, int64_t, false>) kernel_dsv4_rope_fp8_kv_set_rows_f32_i64_t;

template [[host_name("kernel_dsv4_rope_fp8_kv_quantize_norm_f32")]] kernel kernel_dsv4_rope_fp8_kv_quantize_t kernel_dsv4_rope_fp8_kv_quantize<float, false>;
template [[host_name("kernel_dsv4_rope_fp8_kv_quantize_neox_f32")]] kernel kernel_dsv4_rope_fp8_kv_quantize_t kernel_dsv4_rope_fp8_kv_quantize<float, true>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_norm_f32_f16_i32")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f16_i32_t kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int32_t, false>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_neox_f32_f16_i32")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f16_i32_t kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int32_t, true>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_norm_f32_f16_i64")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f16_i64_t kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int64_t, false>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_neox_f32_f16_i64")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f16_i64_t kernel_dsv4_rope_fp8_kv_set_rows<float, half,  int64_t, true>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_norm_f32_f32_i32")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f32_i32_t kernel_dsv4_rope_fp8_kv_set_rows<float, float, int32_t, false>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_neox_f32_f32_i32")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f32_i32_t kernel_dsv4_rope_fp8_kv_set_rows<float, float, int32_t, true>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_norm_f32_f32_i64")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f32_i64_t kernel_dsv4_rope_fp8_kv_set_rows<float, float, int64_t, false>;
template [[host_name("kernel_dsv4_rope_fp8_kv_set_rows_neox_f32_f32_i64")]] kernel kernel_dsv4_rope_fp8_kv_set_rows_f32_i64_t kernel_dsv4_rope_fp8_kv_set_rows<float, float, int64_t, true>;

template<typename T, bool neox>
kernel void kernel_dsv4_rope_tail(
        constant ggml_metal_kargs_dsv4_rope_tail & args,
        device const char * src0,
        device const char * src1,
        device const char * src2,
    ...
    device const T * const src = (device const T *) (src0 + i3*args.nb03 + i2*args.nb02 + i1*args.nb01);
    device       T * dst_data  = (device       T *) (dst  + i3*args.nb3  + i2*args.nb2  + i1*args.nb1);

    for (int i0 = tiitg; i0 < args.n_nope; i0 += tptg.x) {
        dst_data[i0] = src[i0];
    }
    ...
    for (int i0 = 2*tiitg; i0 < args.n_dims; i0 += 2*tptg.x) {
        const int ic = i0/2;
        const float theta = theta_base * pow(args.freq_base, inv_ndims*i0);
        const float freq_factor = args.src2 ? ((device const float *) src2)[ic] : 1.0f;
        ...
        if (neox) {
            const int n_half = args.n_dims/2;
            const int off0 = args.n_nope + ic;
            const int off1 = off0 + n_half;

            const float x0 = src[off0];
            const float x1 = src[off1];

            dst_data[off0] = x0*cos_theta - x1*sin_theta;
            dst_data[off1] = x0*sin_theta + x1*cos_theta;
        } else {
            const int off0 = args.n_nope + i0;
...
typedef decltype(kernel_dsv4_rope_tail<float, false>) kernel_dsv4_rope_tail_t;

template [[host_name("kernel_dsv4_rope_tail_norm_f32")]] kernel kernel_dsv4_rope_tail_t kernel_dsv4_rope_tail<float, false>;
template [[host_name("kernel_dsv4_rope_tail_norm_f16")]] kernel kernel_dsv4_rope_tail_t kernel_dsv4_rope_tail<half,  false>;

template [[host_name("kernel_dsv4_rope_tail_neox_f32")]] kernel kernel_dsv4_rope_tail_t kernel_dsv4_rope_tail<float, true>;
template [[host_name("kernel_dsv4_rope_tail_neox_f16")]] kernel kernel_dsv4_rope_tail_t kernel_dsv4_rope_tail<half,  true>;
...
constant bool FC_flash_attn_ext_use_metal4_qk [[function_constant(FC_FLASH_ATTN_EXT + 11)]];
...
void kernel_flash_attn_ext_impl(
        ...
        ushort  tiisg,
        ushort  sgitg) {
    const ushort NWG = ushort(FC_flash_attn_ext_nwg);

    const ushort iwg = tgpig[2] % NWG;
    const ushort iq3 = tgpig[2] / NWG;
    const uint q_blocks = uint((args.ne01 + Q - 1)/Q);

    uint iq2_u = tgpig[1];
    uint iq1_blk = tgpig[0];

    if (FC_flash_attn_ext_walk_mode == GGML_METAL_MUL_MM_WALK_REGULAR) {
        iq1_blk = tgpig[0] % q_blocks;
        iq2_u   = tgpig[0] / q_blocks;
    } else if (FC_flash_attn_ext_walk_mode == GGML_METAL_MUL_MM_WALK_MORTON) {
        const uint2 tile = ggml_metal_morton_decode_2d(tgpig[0]);
        iq1_blk = tile.x;
        iq2_u   = tile.y;
    }

    if (iq1_blk >= q_blocks || iq2_u >= uint(args.ne02)) {
        return;
    }

    const ushort iq2 = ushort(iq2_u);
    const uint iq1 = iq1_blk*Q;
  ...
  //constexpr short PV16 = PV/16;
...
#ifdef GGML_METAL_HAS_TENSOR
                if (FC_flash_attn_ext_use_metal4_qk && is_same<q_t, half>::value && DK == 128) {
                    constexpr short K_TILE = 32;

                    threadgroup half * skh = (threadgroup half *) sk;

                    auto tQ = tensor<threadgroup half, dextents<int32_t, 2>, tensor_inline>(
                            (threadgroup half *) pq,
                            dextents<int32_t, 2>(DK, Q));
                    auto tK = tensor<threadgroup half, dextents<int32_t, 2>, tensor_inline>(
                            skh,
                            dextents<int32_t, 2>(K_TILE, 8));

                    mpp::tensor_ops::matmul2d<
                        mpp::tensor_ops::matmul2d_descriptor(8, 8, K_TILE, false, true, false, mpp::tensor_ops::matmul2d_descriptor::mode::multiply_accumulate),
                        execution_simdgroups<1>> mm;

                    FOR_UNROLL (short cc = 0; cc < NC; ++cc) {
                        auto cQK = mm.template get_destination_cooperative_tensor<decltype(tQ), decltype(tK), float>();

                        for (short loop_k = 0; loop_k < DK; loop_k += K_TILE) {
                            if (tiisg < 16) {
                                const short row = tiisg/2;
                                const short blk = tiisg%2;

                                device const kd4x4_t * pk4x4 = (device const kd4x4_t *) (k + ((ic + 8*(cc*NSG + sgitg) + row)*args.nb11)) + (loop_k + blk*16)/16;
                                k4x4_t mk;
                                deq_k(pk4x4, 0, mk);

                                threadgroup half4 * dst4 = (threadgroup half4 *) (skh + row*K_TILE + blk*16);
                                dst4[0] = half4((float4) mk[0]);
                                dst4[1] = half4((float4) mk[1]);
                                dst4[2] = half4((float4) mk[2]);
                                dst4[3] = half4((float4) mk[3]);
                            }

                            simdgroup_barrier(mem_flags::mem_threadgroup);

                            auto sQ = tQ.slice(loop_k, 0);
                            auto sK = tK.slice(0, 0);
                            mm.run(sQ, sK, cQK);
                        }

                        auto tScores = tensor<threadgroup float, dextents<int32_t, 2>, tensor_inline>(
                                ps,
                                dextents<int32_t, 2>(SH, 8));
                        cQK.store(tScores);
    ...
    for (short jj = 0; jj < NQ; ++jj) {
        ...
        if (NWG == 1) {
            ...
            device float  * htmp1 = (device float  *) dst + (uint64_t) args.ne3*args.ne2*args.ne1*DV*NWG;

            if (DV4 % NW == 0) {
                FOR_UNROLL (short ii = 0; ii < DV4/NW; ++ii) {
                    const short i = ii*NW + tiisg;

                    htmp4[NWG*i + iwg] = (float4) so4[j*PV4 + i];
                }
            } else {
                for (short i = tiisg; i < DV4; i += NW) {
                    htmp4[NWG*i + iwg] = (float4) so4[j*PV4 + i];
...
template<
    typename q4_t,  // query types in shared memory
    typename k4_t,  // key types after dequantization
    typename v4_t,  // value types in device memory
    typename qk_t,  // unused, kept for signature parity
    typename s_t,
    typename s4_t,
    typename o4_t,
    typename kd4_t,
...
kernel void kernel_flash_attn_ext_vec_metal4(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
    ...
    static_assert(DK == 128, "Metal4 vec attention first pass is tuned for dk=128");
    static_assert(DV == 128, "Metal4 vec attention first pass is tuned for dv=128");
    static_assert(DK % 32 == 0, "DK must be divisible by 32");
    static_assert(DV % 32 == 0, "DV must be divisible by 32");
    static_assert(is_same<q4_t, half4>::value, "Metal4 vec attention expects half4 queries");
...
#define NS20 (FC_flash_attn_ext_vec_ns20)
    ...
    threadgroup half  * shk = (threadgroup half  *) (shmem_f16 + PK + sgitg*(C*DK));
    threadgroup s_t   * ss  = (threadgroup s_t   *) (shmem_f16 + PK + NSG*(C*DK) + sgitg*SH);
    threadgroup s4_t  * ss4 = (threadgroup s4_t  *) (shmem_f16 + PK + NSG*(C*DK) + sgitg*SH);
    threadgroup half  * sm  = (threadgroup half  *) (shmem_f16 + PK + NSG*(C*DK) + sgitg*SH + 2*C);
    threadgroup o4_t  * so4 = (threadgroup o4_t  *) (shmem_f16 + PK + NSG*(C*DK) + NSG*SH + 2*sgitg*PV);
    ...
    auto tQ = tensor<threadgroup half, dextents<int32_t, 2>, tensor_inline>(
            (threadgroup half *) sq4,
            dextents<int32_t, 2>(DK, 1));
    auto tK = tensor<threadgroup half, dextents<int32_t, 2>, tensor_inline>(
            shk,
            dextents<int32_t, 2>(DK, C));

    mpp::tensor_ops::matmul2d<
        mpp::tensor_ops::matmul2d_descriptor(1, C, DK, false, true, false, mpp::tensor_ops::matmul2d_descriptor::mode::multiply_accumulate),
        execution_simdgroups<1>> mm;

    {
        ...
        for (int ic0 = iwg*NSG + sgitg; ; ic0 += NWG*NSG) {
            ...
            {
                threadgroup half4 * shk4 = (threadgroup half4 *) shk;
                constexpr int TILE_Q4 = C*DK4;

                for (int i = tiisg; i < TILE_Q4; i += NW) {
                    const int cc = i / DK4;
                    const short ik = i % DK4;

                    device const kd4_t * pk = (device const kd4_t *) (k + (ic + cc)*args.nb11);
                    k4_t mk;
                    deq_k_t4(pk + ik/nl_k, ik%nl_k, mk);
                    shk4[i] = half4((float4) mk);
                }
            }
...

            {
                ...
                auto sK = tK.slice(0, 0);
                auto tScores = tensor<threadgroup float, dextents<int32_t, 2>, tensor_inline>(ss, dextents<int32_t, 2>(C, 1));
                auto cQK = mm.template get_destination_cooperative_tensor<decltype(tQ), decltype(tK), float>();
                ...
                const float raw = ss[tiisg];

                if (FC_flash_attn_ext_vec_has_mask &&
                   !FC_flash_attn_ext_vec_has_scap &&
                   !FC_flash_attn_ext_vec_has_bias) {
                    ss[tiisg] = fma(raw, args.scale, (float) sm[tiisg]);
                } else {
                    float score = raw*args.scale;
                    ...
                    if (FC_flash_attn_ext_vec_has_bias) {
                        score += (float) sm[tiisg]*slope;
                    } else {
                        score += (float) sm[tiisg];
                    }
...
#ifdef GGML_METAL_HAS_TENSOR
typedef decltype(kernel_flash_attn_ext_vec_metal4<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 128, 128, 1>) flash_attn_ext_vec_metal4_t;

template [[host_name("kernel_flash_attn_ext_vec_metal4_f32_dk128_dv128")]]  kernel flash_attn_ext_vec_metal4_t kernel_flash_attn_ext_vec_metal4<FA_TYPES_F32, float4,  1, dequantize_f32_t4,  float4,   1, dequantize_f32_t4,  128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_metal4_f16_dk128_dv128")]]  kernel flash_attn_ext_vec_metal4_t kernel_flash_attn_ext_vec_metal4<FA_TYPES,     half4,   1, dequantize_f16_t4,  half4,    1, dequantize_f16_t4,  128, 128, 1>;
#if defined(GGML_METAL_HAS_BF16)
template [[host_name("kernel_flash_attn_ext_vec_metal4_bf16_dk128_dv128")]] kernel flash_attn_ext_vec_metal4_t kernel_flash_attn_ext_vec_metal4<FA_TYPES,     bfloat4, 1, dequantize_bf16_t4, bfloat4,  1, dequantize_bf16_t4, 128, 128, 1>;
#endif
#endif
...
template [[host_name("kernel_flash_attn_ext_vec_f16_dk512_dv512")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES,     half4,      1, dequantize_f16_t4,  half4,       1, dequantize_f16_t4,  512, 512, 2>;
...
#define DV  (FC_flash_attn_ext_vec_reduce_DV)
    ...
    const bool active_lane = iwg < NWG;

    device const float  * ss    = (device const float  *) htmp + (uint64_t)args.nrows*DV*NWG;

    float S = active_lane ? ss[rid*(2*NWG) + 2*iwg + 0] : 0.0f;
    float M = active_lane ? ss[rid*(2*NWG) + 2*iwg + 1] : -INFINITY;
    ...
    for (short i = sgitg; i < DV4; i += NWG) {
        const float4 partial = active_lane ? htmp4[i*NWG + iwg] : float4(0.0f);
...
template<int nr0, typename args_t>
void kernel_mul_mv_iq2_xxs_f32_pair_impl(
        args_t args,
    ...
    device const block_iq2_xxs * xa = (device const block_iq2_xxs *) (src0a + offset0);
    device const block_iq2_xxs * xb = (device const block_iq2_xxs *) (src0b + offset0);
    device const float         * y  = (device const float         *) (src1 + offset1);
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        device const block_iq2_xxs * xra = xa + ibl;
        device const block_iq2_xxs * xrb = xb + ibl;
        device const uint16_t * q2a = xra->qs + 4 * ib;
        device const uint16_t * q2b = xrb->qs + 4 * ib;
        device const half * dha = &xra->d;
        device const half * dhb = &xrb->d;

        for (short row = 0; row < nr0; row++) {
            const float dba = dha[0];
            const float dbb = dhb[0];

            device const uint8_t * aux8a = (device const uint8_t *)q2a;
            device const uint8_t * aux8b = (device const uint8_t *)q2b;
            const uint32_t aux32a = q2a[2] | (q2a[3] << 16);
            const uint32_t aux32b = q2b[2] | (q2b[3] << 16);
            const float da = dba * (0.5f + (aux32a >> 28));
            const float db = dbb * (0.5f + (aux32b >> 28));

            float suma = 0.0f;
            float sumb = 0.0f;
            for (short l = 0; l < 4; ++l) {
                const threadgroup uint8_t * grida = (const threadgroup uint8_t *)(svalues + aux8a[l]);
                const threadgroup uint8_t * gridb = (const threadgroup uint8_t *)(svalues + aux8b[l]);
                const uint8_t signsa = ssigns[(aux32a >> 7*l) & 127];
                const uint8_t signsb = ssigns[(aux32b >> 7*l) & 127];
                for (short j = 0; j < 8; ++j) {
                    const float yv = yl[8*l + j];
                    suma += yv * grida[j] * (signsa & kmask_iq2xs[j] ? -1.f : 1.f);
                    sumb += yv * gridb[j] * (signsb & kmask_iq2xs[j] ? -1.f : 1.f);
                }
            }
            sumfa[row] += da * suma;
            sumfb[row] += db * sumb;
    ...
    device float * dsta_f32 = (device float *) dsta + (uint64_t)im*args.ne0*args.ne1 + (uint64_t)r1*args.ne0;
    device float * dstb_f32 = (device float *) dstb + (uint64_t)im*args.ne0*args.ne1 + (uint64_t)r1*args.ne0;

    for (int row = 0; row < nr0 && first_row + row < args.ne0; ++row) {
        const float suma = simd_sum(sumfa[row]) * 0.25f;
        const float sumb = simd_sum(sumfb[row]) * 0.25f;
        if (tiisg == 0) {
            dsta_f32[first_row + row] = suma;
            dstb_f32[first_row + row] = sumb;
        }
    }
}

[[host_name("kernel_mul_mv_pair_iq2_xxs_f32")]]
kernel void kernel_mul_mv_pair_iq2_xxs_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0a,
        device const char * src0b,
        device const char * src1,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_iq2_xxs_f32_pair_impl<N_R0_IQ2_XXS, constant ggml_metal_kargs_mul_mv &>(args, src0a, src0b, src1, dsta, dstb, shmem, tgpig, tiisg, sgitg);
}

[[host_name("kernel_mul_mv_pair_limited_swiglu_iq2_xxs_f32")]]
kernel void kernel_mul_mv_pair_limited_swiglu_iq2_xxs_f32(
        constant ggml_metal_kargs_mul_mv & args,
        constant float & limit,
        device const char * src0_gate,
        device const char * src0_up,
        device const char * src1,
        device       char * dst,
    ...
    const int first_row = (r0 * NSG + sgitg) * N_R0_IQ2_XXS;
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        device const block_iq2_xxs * xrg = xg + ibl;
        device const block_iq2_xxs * xru = xu + ibl;
        device const uint16_t * q2g = xrg->qs + 4 * ib;
        device const uint16_t * q2u = xru->qs + 4 * ib;
        device const half * dhg = &xrg->d;
        device const half * dhu = &xru->d;

        for (short row = 0; row < N_R0_IQ2_XXS; row++) {
            const float dbg = dhg[0];
            const float dbu = dhu[0];

            device const uint8_t * aux8g = (device const uint8_t *)q2g;
            device const uint8_t * aux8u = (device const uint8_t *)q2u;
            const uint32_t aux32g = q2g[2] | (q2g[3] << 16);
            const uint32_t aux32u = q2u[2] | (q2u[3] << 16);
            const float dg = dbg * (0.5f + (aux32g >> 28));
            const float du = dbu * (0.5f + (aux32u >> 28));
            ...
            for (short l = 0; l < 4; ++l) {
                ...
                const uint8_t signsg = ssigns[(aux32g >> 7*l) & 127];
                const uint8_t signsu = ssigns[(aux32u >> 7*l) & 127];
                for (short j = 0; j < 8; ++j) {
                    const float yv = yl[8*l + j];
                    sg += yv * gridg[j] * (signsg & kmask_iq2xs[j] ? -1.f : 1.f);
                    su += yv * gridu[j] * (signsu & kmask_iq2xs[j] ? -1.f : 1.f);
                }
            }
            sumg[row] += dg * sg;
            sumu[row] += du * su;
    ...
    for (int row = 0; row < N_R0_IQ2_XXS && first_row + row < args.ne0; ++row) {
        const float gate = simd_sum(sumg[row]) * 0.25f;
        const float up   = simd_sum(sumu[row]) * 0.25f;
        if (tiisg == 0) {
            const float gate_limited = min(gate, limit);
            const float up_limited = clamp(up, -limit, limit);
            dst_f32[first_row + row] = gate_limited/(1.0f + exp(-gate_limited))*up_limited;
        }
    }
}

[[host_name("kernel_mul_mv_pair_limited_swiglu_weighted_iq2_xxs_f32")]]
kernel void kernel_mul_mv_pair_limited_swiglu_weighted_iq2_xxs_f32(
        constant ggml_metal_kargs_mul_mv & args,
        constant float & limit,
        device const char * src0_gate,
        device const char * src0_up,
    ...
    for (int row = 0; row < N_R0_IQ2_XXS && first_row + row < args.ne0; ++row) {
        ...
        if (tiisg == 0) {
            const float gate_limited = min(gate, limit);
            const float up_limited = clamp(up, -limit, limit);
            const float swiglu = gate_limited/(1.0f + exp(-gate_limited))*up_limited;
            dst_f32[first_row + row] = swiglu*route_weight;
        }
    }
}
...
[[host_name("kernel_dsv4_ffn_moe_stage_gate_up_iq2_xxs_f32")]]
kernel void kernel_dsv4_ffn_moe_stage_gate_up_iq2_xxs_f32(
        constant ggml_metal_kargs_mul_mv_id & args,
        constant float & limit,
        constant ggml_metal_dsv4_ffn_moe_stage_v2_gate_args & v2,
        device const char * src0_gate,
        device const char * src0_up,
        device const char * src1,
        device       char * dst,
    ...
    device const float * y =
        (device const float *)(src1 + (uint64_t) i11 * args.nb11 + (uint64_t) i12 * args.nb12);
    ...
    device float * dst_gate_f32 = (device float *)(dst + ((uint64_t) i12 * 24u + (uint64_t) idx) * v2.slot_stride);
    device float * dst_up_f32   = (device float *)(dst + ((uint64_t) i12 * 24u + (uint64_t) 6 + (uint64_t) idx) * v2.slot_stride);
    device float * dst_mid_f32  = (device float *)(dst + ((uint64_t) i12 * 24u + (uint64_t) 12 + (uint64_t) idx) * v2.slot_stride);
    for (int row = 0; row < N_R0_IQ2_XXS && first_row + row < args.ne0; ++row) {
        const float gate = simd_sum(sumg[row]) * 0.25f;
        const float up   = simd_sum(sumu[row]) * 0.25f;
        if (tiisg == 0) {
            ...
            float swiglu;
            if (v2.swiglu_formula_mode == 1 || v2.swiglu_formula_mode == 3 || v2.swiglu_formula_mode == 4) {
                const float x0 = g;
                const float x1 = u;
                const float silu = x0 / (1.0f + exp(-x0));
                swiglu = silu * x1;
            } else if (v2.swiglu_formula_mode == 2) {
                const float x0 = g;
                const float x1 = u;
                const float silu = x0 / (1.0f + fast::exp(-x0));
                swiglu = silu * x1;
            } else {
                swiglu = (g / (1.0f + exp(-g))) * u;
            }
            dst_mid_f32[out_row] = swiglu;
        }
    }
...
[[host_name("kernel_dsv4_ffn_moe_stage_weight_f32")]]
kernel void kernel_dsv4_ffn_moe_stage_weight_f32(
        constant ggml_metal_dsv4_ffn_moe_stage_v2_weight_args & args,
        device       char * dst,
        device const char * weights,
        uint gid [[thread_position_in_grid]]) {
    const uint32_t total = args.n_ff * 6u;
    if (gid >= total) {
        return;
    }

    const uint32_t slot = gid / args.n_ff;
    const uint32_t row  = gid - slot * args.n_ff;
    device float * mid = (device float *)(dst + (uint64_t)(12u + slot) * args.slot_stride);
    device const float * route_w = (device const float *)(weights + (uint64_t) slot * args.weight_slot_stride);
    mid[row] = mid[row] * route_w[0];
}

[[host_name("kernel_dsv4_routed_moe_swiglu_slots_f32")]]
kernel void kernel_dsv4_routed_moe_swiglu_slots_f32(
        constant ggml_metal_dsv4_ffn_moe_stage_v2_weight_args & args,
        device char * dst,
        uint gid [[thread_position_in_grid]]) {
    const uint32_t total = args.n_ff * 6u;
    ...
    const uint32_t slot = gid / args.n_ff;
    const uint32_t row  = gid - slot * args.n_ff;
    device const float * gate = (device const float *)(dst + (uint64_t) slot * args.slot_stride);
    device const float * up   = (device const float *)(dst + (uint64_t)(6u + slot) * args.slot_stride);
    device       float * mid  = (device       float *)(dst + (uint64_t)(12u + slot) * args.slot_stride);

    const float x0 = gate[row];
    const float x1 = up[row];
    const float silu = x0 / (1.0f + exp(-x0));
    mid[row] = silu * x1;
}

[[host_name("kernel_dsv4_routed_moe_one_tensor_decode_dryrun")]]
kernel void kernel_dsv4_routed_moe_one_tensor_decode_dryrun(
...
[[host_name("kernel_dsv4_decode_layer_executor_dryrun")]]
kernel void kernel_dsv4_decode_layer_executor_dryrun(
        constant int & layer [[buffer(0)]],
        constant int & token [[buffer(1)]],
        constant int & flags [[buffer(2)]],
...
// Passthrough copy: dst[i] = src0[i] for i in [0, total_elems_f32).
// Assumes f32, contiguous src0 and dst with identical layouts (the helper
// allocates dst via ggml_dup_tensor so this holds by construction at T104).
// T105 will replace the body with per-stage dispatch driven by stage_mask.
[[host_name("kernel_dsv4_decode_layer_stub")]]
kernel void kernel_dsv4_decode_layer_stub(
        constant ggml_metal_kargs_dsv4_decode_layer & args [[buffer(0)]],
        device const float * src0 [[buffer(1)]],
        device       float * dst  [[buffer(2)]],
        uint gid [[thread_position_in_grid]]) {
    if ((ulong) gid < args.total_elems_f32) {
        dst[gid] = src0[gid];
    }
}
...
template<int nr0, typename args_t>
void kernel_mul_mv_iq3_xxs_f32_pair_impl(
        args_t args,
        device const char * src0a,
        device const char * src0b,
        device const char * src1,
    ...
    device const block_iq3_xxs * xa = (device const block_iq3_xxs *) (src0a + offset0);
    device const block_iq3_xxs * xb = (device const block_iq3_xxs *) (src0b + offset0);
    device const float         * y  = (device const float         *) (src1 + offset1);
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        device const block_iq3_xxs * xra = xa + ibl;
        device const block_iq3_xxs * xrb = xb + ibl;
        device const uint8_t  * q3a = xra->qs + 8 * ib;
        device const uint8_t  * q3b = xrb->qs + 8 * ib;
        device const uint16_t * gasa = (device const uint16_t *)(xra->qs + QK_K/4) + 2 * ib;
        device const uint16_t * gasb = (device const uint16_t *)(xrb->qs + QK_K/4) + 2 * ib;
        device const half * dha = &xra->d;
        device const half * dhb = &xrb->d;
        ...
        for (short row = 0; row < nr0; row++) {
            const float dba = dha[0];
            const float dbb = dhb[0];
            const uint32_t aux32a = gasa[0] | (gasa[1] << 16);
            const uint32_t aux32b = gasb[0] | (gasb[1] << 16);
            const float da = dba * (0.5f + (aux32a >> 28));
            const float db = dbb * (0.5f + (aux32b >> 28));
            ...
            float2 sumb = {0};
            for (short l = 0; l < 4; ++l) {
                const threadgroup uint8_t * grid1a = (const threadgroup uint8_t *)(svalues + q3a[2*l+0]);
                const threadgroup uint8_t * grid2a = (const threadgroup uint8_t *)(svalues + q3a[2*l+1]);
                const threadgroup uint8_t * grid1b = (const threadgroup uint8_t *)(svalues + q3b[2*l+0]);
                const threadgroup uint8_t * grid2b = (const threadgroup uint8_t *)(svalues + q3b[2*l+1]);
                const uint8_t signsa = ssigns[(aux32a >> 7*l) & 127];
                const uint8_t signsb = ssigns[(aux32b >> 7*l) & 127];
                for (short j = 0; j < 4; ++j) {
                    const float y0 = yl[8*l + j + 0];
                    const float y1 = yl[8*l + j + 4];
                    suma[0] += y0 * grid1a[j] * (signsa & kmask_iq2xs[j+0] ? -1.f : 1.f);
                    suma[1] += y1 * grid2a[j] * (signsa & kmask_iq2xs[j+4] ? -1.f : 1.f);
                    sumb[0] += y0 * grid1b[j] * (signsb & kmask_iq2xs[j+0] ? -1.f : 1.f);
                    sumb[1] += y1 * grid2b[j] * (signsb & kmask_iq2xs[j+4] ? -1.f : 1.f);
                }
            }
            sumfa[row] += da * (suma[0] + suma[1]);
            sumfb[row] += db * (sumb[0] + sumb[1]);
    ...
    for (int row = 0; row < nr0 && first_row + row < args.ne0; ++row) {
        const float suma = simd_sum(sumfa[row]) * 0.5f;
        const float sumb = simd_sum(sumfb[row]) * 0.5f;
        if (tiisg == 0) {
            dsta_f32[first_row + row] = suma;
            dstb_f32[first_row + row] = sumb;
        }
...
[[host_name("kernel_mul_mv_pair_iq3_xxs_f32")]]
kernel void kernel_mul_mv_pair_iq3_xxs_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0a,
        device const char * src0b,
        device const char * src1,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_iq3_xxs_f32_pair_impl<N_R0_IQ3_XXS, constant ggml_metal_kargs_mul_mv &>(args, src0a, src0b, src1, dsta, dstb, shmem, tgpig, tiisg, sgitg);
}
...
void kernel_mul_mv_f8_e4m3_b128_f32_impl(
    ...
    for (int ib = 0; ib < nb && ib < ns01; ++ib) {
        ...
        FOR_UNROLL (short row = 0; row < NR0; ++row) {
            ...
            device const uint8_t * q = xb.qs + qoff;
            const float d = e8m0_to_fp32(xb.e);
...
kernel void kernel_dsv4_kv_finalize_decode(
        constant ggml_metal_kargs_dsv4_kv_finalize_decode & args,
        device const char * src,
        device       char * cache,
        device const char * rows,
        device       char * dst,
        uint tid [[thread_position_in_grid]]) {
    const uint64_t total = uint64_t(args.width) * uint64_t(args.n_rows);
    if (uint64_t(tid) >= total) {
        return;
    }

    const int32_t i = int32_t(tid % uint(args.width));
    const int32_t r = int32_t(tid / uint(args.width));

    const float v = *(device const float *) (src + uint64_t(i)*args.src_nb0 + uint64_t(r)*args.src_nb2);
    *(device float *) (dst + uint64_t(i)*args.dst_nb0 + uint64_t(r)*args.dst_nb2) = v;

    if (args.dry_run == 0) {
        const TI row = *(device const TI *) (rows + uint64_t(r)*args.rows_nb0);
        *(device TC *) (cache + uint64_t(row)*args.cache_nb1 + uint64_t(i)*args.cache_nb0) = TC(v);
    }
}

typedef decltype(kernel_dsv4_kv_finalize_decode<half,  int32_t>) kernel_dsv4_kv_finalize_decode_f16_i32_t;
typedef decltype(kernel_dsv4_kv_finalize_decode<half,  int64_t>) kernel_dsv4_kv_finalize_decode_f16_i64_t;
typedef decltype(kernel_dsv4_kv_finalize_decode<float, int32_t>) kernel_dsv4_kv_finalize_decode_f32_i32_t;
typedef decltype(kernel_dsv4_kv_finalize_decode<float, int64_t>) kernel_dsv4_kv_finalize_decode_f32_i64_t;

template [[host_name("kernel_dsv4_kv_finalize_decode_f16_i32")]] kernel kernel_dsv4_kv_finalize_decode_f16_i32_t kernel_dsv4_kv_finalize_decode<half,  int32_t>;
template [[host_name("kernel_dsv4_kv_finalize_decode_f16_i64")]] kernel kernel_dsv4_kv_finalize_decode_f16_i64_t kernel_dsv4_kv_finalize_decode<half,  int64_t>;
template [[host_name("kernel_dsv4_kv_finalize_decode_f32_i32")]] kernel kernel_dsv4_kv_finalize_decode_f32_i32_t kernel_dsv4_kv_finalize_decode<float, int32_t>;
template [[host_name("kernel_dsv4_kv_finalize_decode_f32_i64")]] kernel kernel_dsv4_kv_finalize_decode_f32_i64_t kernel_dsv4_kv_finalize_decode<float, int64_t>;
...
constant short FC_mul_mm_walk_mode [[function_constant(FC_MUL_MM + 2)]];
...
static inline uint2 ggml_metal_morton_decode_2d(uint code) {
    return uint2(ggml_metal_compact1by1(code), ggml_metal_compact1by1(code >> 1));
}

// each block_q contains 16*nl weights
template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, short nk, bool tensor_reduced_precision, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4, typename TDst>
void kernel_mul_mm_impl(
    ...
    threadgroup S1 * sb = (threadgroup S1 *)(shmem + sizeof(S0) * NR0 * NK);
    ...
    const uint grid_x = uint((args.ne1 + NR1 - 1)/NR1);
    const uint grid_y = uint((args.ne0 + NR0 - 1)/NR0);

    uint tile_x = tgpig.x;
    uint tile_y = tgpig.y;

    if (FC_mul_mm_walk_mode == GGML_METAL_MUL_MM_WALK_REGULAR) {
        tile_x = tgpig.x % grid_x;
        tile_y = tgpig.x / grid_x;
    } else if (FC_mul_mm_walk_mode == GGML_METAL_MUL_MM_WALK_MORTON) {
        const uint2 tile = ggml_metal_morton_decode_2d(tgpig.x);
        tile_x = tile.x;
        tile_y = tile.y;
    }

    if (tile_x >= grid_x || tile_y >= grid_y) {
        return;
    }

    const int im = tgpig.z;
    const int r0 = int(tile_y)*NR0;
    const int r1 = int(tile_x)*NR1;
    ...
    mpp::tensor_ops::matmul2d<
        mpp::tensor_ops::matmul2d_descriptor(NR1, NR0, NK, false, true, tensor_reduced_precision, mpp::tensor_ops::matmul2d_descriptor::mode::multiply_accumulate),
    ...
    const bool use_direct_float_store =
            is_same<TDst, float>::value &&
            (!FC_mul_mm_bc_out || (r0 + NR0 <= args.ne0 && r1 + NR1 <= args.ne1));

    if (use_direct_float_store) {
        ...
        // use a threadgroup float tile before storing to the final destination. this is required
        // for partial tiles and for f16 output, where we downcast only once per output element.
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        if (sgitg == 0) {
            for (int j = tiitg; j < nr1; j += NR1) {
                device TDst * D = (device TDst *) dst + r0 + (r1 + j)*args.ne0 + im*args.ne1*args.ne0;
...
template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, short nk, bool tensor_reduced_precision, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm(
        ...
        ushort tiitg[[thread_index_in_threadgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mm_impl<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, block_q, nl, nk, tensor_reduced_precision, dequantize_func, T0, T0_4x4, T1, T1_2x4, float>(
            args, src0, src1, dst, shmem, tgpig, tiitg, sgitg);
}

template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, short nk, bool tensor_reduced_precision, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm_flashmoe_metal4(
        constant ggml_metal_kargs_mul_mm & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, short nk, bool tensor_reduced_precision, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm_out_f16(
        constant ggml_metal_kargs_mul_mm & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort tiitg[[thread_index_in_threadgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mm_impl<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, block_q, nl, nk, tensor_reduced_precision, dequantize_func, T0, T0_4x4, T1, T1_2x4, half>(
            args, src0, src1, dst, shmem, tgpig, tiitg, sgitg);
}

template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, short nk, bool tensor_reduced_precision, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm_flashmoe_metal4_out_f16(
        constant ggml_metal_kargs_mul_mm & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
...
typedef decltype(kernel_mul_mm<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, float4x4, 1, 32, false, dequantize_f32, float, float4x4, float, float2x4>) mul_mm_t;
typedef decltype(kernel_mul_mm_flashmoe_metal4<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, float4x4, 1, 32, false, dequantize_f32, float, float4x4, float, float2x4>) flashmoe_metal4_mul_mm_t;
typedef decltype(kernel_mul_mm_out_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, float4x4, 1, 32, false, dequantize_f32, float, float4x4, float, float2x4>) mul_mm_out_f16_t;
typedef decltype(kernel_mul_mm_flashmoe_metal4_out_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, float4x4, 1, 32, false, dequantize_f32, float, float4x4, float, float2x4>) flashmoe_metal4_mul_mm_out_f16_t;
#ifdef GGML_METAL_HAS_TENSOR
...
#endif
#define GGML_METAL_INSTANTIATE_MUL_MM(NAME, S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, DEQ, T0, T0_4x4, T1, T1_2x4) \
template [[host_name("kernel_mul_mm_" NAME)]] kernel mul_mm_t kernel_mul_mm<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, 32, false, DEQ, T0, T0_4x4, T1, T1_2x4>;

#define GGML_METAL_INSTANTIATE_MUL_MM_NK(NAME, NK, S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, DEQ, T0, T0_4x4, T1, T1_2x4) \
template [[host_name("kernel_mul_mm_nk" #NK "_" NAME)]] kernel decltype(kernel_mul_mm<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, NK, false, DEQ, T0, T0_4x4, T1, T1_2x4>) kernel_mul_mm<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, NK, false, DEQ, T0, T0_4x4, T1, T1_2x4>;

#define GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM(NAME, S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, DEQ, T0, T0_4x4, T1, T1_2x4) \
template [[host_name("kernel_mul_mm_flashmoe_metal4_" NAME)]] kernel flashmoe_metal4_mul_mm_t kernel_mul_mm_flashmoe_metal4<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, 32, false, DEQ, T0, T0_4x4, T1, T1_2x4>;

#define GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16(NAME, S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, DEQ, T0, T0_4x4, T1, T1_2x4) \
template [[host_name("kernel_mul_mm_out_f16_" NAME)]] kernel mul_mm_out_f16_t kernel_mul_mm_out_f16<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, 32, false, DEQ, T0, T0_4x4, T1, T1_2x4>;

#define GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16(NAME, S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, DEQ, T0, T0_4x4, T1, T1_2x4) \
template [[host_name("kernel_mul_mm_flashmoe_metal4_out_f16_" NAME)]] kernel flashmoe_metal4_mul_mm_out_f16_t kernel_mul_mm_flashmoe_metal4_out_f16<S0, S0_4x4, S0_8x8, S1, S1_2x4, S1_8x8, BLOCK_Q, NL, 32, false, DEQ, T0, T0_4x4, T1, T1_2x4>;
#ifdef GGML_METAL_HAS_TENSOR
...
GGML_METAL_INSTANTIATE_MUL_MM("f32_f32",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   float4x4,      1,     dequantize_f32,     float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("f16_f32",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   half4x4,       1,     dequantize_f16,     half,   half4x4,   float, float2x4)
#if defined(GGML_METAL_HAS_BF16)
GGML_METAL_INSTANTIATE_MUL_MM("bf16_f32",    bfloat, bfloat4x4, simdgroup_bfloat8x8, bfloat, bfloat2x4, simdgroup_bfloat8x8, bfloat4x4,     1,     dequantize_bf16,    bfloat, bfloat4x4, float, float2x4)
#endif
GGML_METAL_INSTANTIATE_MUL_MM("q4_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_0,    2,     dequantize_q4_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q4_1_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_1,    2,     dequantize_q4_1,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_0,    2,     dequantize_q5_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_1_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_1,    2,     dequantize_q5_1,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q8_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q8_0,    2,     dequantize_q8_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("mxfp4_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_mxfp4,   2,     dequantize_mxfp4,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("f8_e4m3_b128_f32", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_f8_e4m3_b128, 8, dequantize_f8_e4m3_b128, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q2_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_K,    QK_NL, dequantize_q2_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q3_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q3_K,    QK_NL, dequantize_q3_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q4_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_K,    QK_NL, dequantize_q4_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_K,    QK_NL, dequantize_q5_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q6_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q6_K,    QK_NL, dequantize_q6_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 64, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 64, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 64, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 48, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 48, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 48, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 80, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 80, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 80, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 96, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 96, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 96, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 112, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 112, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 112, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q4_K_f32", 128, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q5_K_f32", 128, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_NK("q6_K_f32", 128, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_xxs_f32", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xxs, QK_NL, dequantize_iq2_xxs, float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_xs_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xs,  QK_NL, dequantize_iq2_xs,  float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq3_xxs_f32", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_xxs, QK_NL, dequantize_iq3_xxs, float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq3_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_s,   QK_NL, dequantize_iq3_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_s,   QK_NL, dequantize_iq2_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq1_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_s,   QK_NL, dequantize_iq1_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq1_m_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_m,   QK_NL, dequantize_iq1_m,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq4_nl_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_nl,  2,     dequantize_iq4_nl,  float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq4_xs_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_xs,  QK_NL, dequantize_iq4_xs,  float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("f32_f32",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   float4x4,      1,     dequantize_f32,     float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("f16_f32",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   half4x4,       1,     dequantize_f16,     half,   half4x4,   float, float2x4)
#if defined(GGML_METAL_HAS_BF16)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("bf16_f32",    bfloat, bfloat4x4, simdgroup_bfloat8x8, bfloat, bfloat2x4, simdgroup_bfloat8x8, bfloat4x4,     1,     dequantize_bf16,    bfloat, bfloat4x4, float, float2x4)
#endif
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_0,    2,     dequantize_q4_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_1_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_1,    2,     dequantize_q4_1,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_0,    2,     dequantize_q5_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_1_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_1,    2,     dequantize_q5_1,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q8_0_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q8_0,    2,     dequantize_q8_0,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("mxfp4_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_mxfp4,   2,     dequantize_mxfp4,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("f8_e4m3_b128_f32", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_f8_e4m3_b128, 8, dequantize_f8_e4m3_b128, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q2_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_K,    QK_NL, dequantize_q2_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q3_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q3_K,    QK_NL, dequantize_q3_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_K,    QK_NL, dequantize_q4_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q4_K_f32", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_K,    QK_NL, dequantize_q5_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q5_K_f32", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q6_K_f32",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q6_K,    QK_NL, dequantize_q6_K,    float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q6_K_f32", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_0,    2,     dequantize_q4_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_1_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_1,    2,     dequantize_q4_1,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_0,    2,     dequantize_q5_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_1_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_1,    2,     dequantize_q5_1,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q8_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q8_0,    2,     dequantize_q8_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q2_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_K,    QK_NL, dequantize_q2_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q3_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q3_K,    QK_NL, dequantize_q3_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q4_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_K,    QK_NL, dequantize_q4_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q4_K_f16", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q5_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_K,    QK_NL, dequantize_q5_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q5_K_f16", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q5_K, QK_NL, dequantize_q5_K, float, float4x4, half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("q6_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q6_K,    QK_NL, dequantize_q6_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16("q6_K_f16", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q6_K, QK_NL, dequantize_q6_K, float, float4x4, half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq2_xxs_f32", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xxs, QK_NL, dequantize_iq2_xxs, float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq2_xs_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xs,  QK_NL, dequantize_iq2_xs,  float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq3_xxs_f32", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_xxs, QK_NL, dequantize_iq3_xxs, float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq3_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_s,   QK_NL, dequantize_iq3_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq2_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_s,   QK_NL, dequantize_iq2_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq1_s_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_s,   QK_NL, dequantize_iq1_s,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq1_m_f32",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_m,   QK_NL, dequantize_iq1_m,   float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq4_nl_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_nl,  2,     dequantize_iq4_nl,  float,  float4x4,  float, float2x4)
GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16("iq4_xs_f32",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_xs,  QK_NL, dequantize_iq4_xs,  float,  float4x4,  float, float2x4)

GGML_METAL_INSTANTIATE_MUL_MM("f32_f16",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   float4x4,      1,     dequantize_f32,     float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("f16_f16",     half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   half4x4,       1,     dequantize_f16,     half,   half4x4,   half, half2x4)
GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM("f16_f16", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, half4x4, 1, dequantize_f16, half, half4x4, half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q4_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_0,    2,     dequantize_q4_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q4_1_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_1,    2,     dequantize_q4_1,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_0,    2,     dequantize_q5_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_1_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_1,    2,     dequantize_q5_1,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q8_0_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q8_0,    2,     dequantize_q8_0,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("mxfp4_f16",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_mxfp4,   2,     dequantize_mxfp4,   float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("f8_e4m3_b128_f16", half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_f8_e4m3_b128, 8, dequantize_f8_e4m3_b128, float, float4x4, half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q2_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_K,    QK_NL, dequantize_q2_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q3_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q3_K,    QK_NL, dequantize_q3_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q4_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_K,    QK_NL, dequantize_q4_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q5_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q5_K,    QK_NL, dequantize_q5_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("q6_K_f16",    half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q6_K,    QK_NL, dequantize_q6_K,    float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_xxs_f16", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xxs, QK_NL, dequantize_iq2_xxs, float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_xs_f16",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_xs,  QK_NL, dequantize_iq2_xs,  float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq3_xxs_f16", half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_xxs, QK_NL, dequantize_iq3_xxs, float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq3_s_f16",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq3_s,   QK_NL, dequantize_iq3_s,   float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq2_s_f16",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq2_s,   QK_NL, dequantize_iq2_s,   float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq1_s_f16",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_s,   QK_NL, dequantize_iq1_s,   float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq1_m_f16",   half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq1_m,   QK_NL, dequantize_iq1_m,   float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq4_nl_f16",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_nl,  2,     dequantize_iq4_nl,  float,  float4x4,  half, half2x4)
GGML_METAL_INSTANTIATE_MUL_MM("iq4_xs_f16",  half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_iq4_xs,  QK_NL, dequantize_iq4_xs,  float,  float4x4,  half, half2x4)
#ifdef GGML_METAL_HAS_TENSOR
...
#undef GGML_METAL_INSTANTIATE_MUL_MM
#undef GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM
#undef GGML_METAL_INSTANTIATE_MUL_MM_OUT_F16
#undef GGML_METAL_INSTANTIATE_FLASHMOE_METAL4_MUL_MM_OUT_F16
#ifdef GGML_METAL_HAS_TENSOR
...
[[host_name("kernel_dsv4_attn_out_low_q8_0_f32")]]
kernel void kernel_dsv4_attn_out_low_q8_0_f32(
    ...
    device const char * src0_cur = src0s + (uint64_t) idx*args.nb02;
    device const char * src1_cur = src1  + (uint64_t) i11*args.nb11 + (uint64_t) i12*args.nb12;
    device       char * dst_cur  = dst   + ((uint64_t) idx*args.ne0 + (uint64_t) i12*args.ne1*args.ne0)*sizeof(float);
    ...
    kernel_mul_mv_q8_0_f32_impl<N_R0_Q8_0, thread ggml_metal_kargs_mul_mv &>(
            args0, src0_cur, src1_cur, dst_cur, shmem, tgpig, tiisg, sgitg);
}

[[host_name("kernel_mul_mv_id_sum6_q2_K_f32")]]
kernel void kernel_mul_mv_id_sum6_q2_K_f32(
        constant ggml_metal_kargs_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    for (int expert_slot = 0; expert_slot < 6; expert_slot++) {
        ...
        device const block_q2_K * x = (device const block_q2_K *)(src0s + (uint64_t) expert*args.nb02 + (uint64_t) first_row*args.nb01);
        device const float * y = (device const float *)(token_src1 + (uint64_t) expert_slot*args.nb11);
        device const float * y4 = y + ix * QK_K + 128 * iq + 8 * ir;
        thread float * sumf = expert_slot == 0 ? sumf0 :
                expert_slot == 1 ? sumf1 :
                expert_slot == 2 ? sumf2 :
                expert_slot == 3 ? sumf3 :
                expert_slot == 4 ? sumf4 : sumf5;
    ...
    for (int row = 0; row < nr0 && first_row + row < args.ne0; row++) {
        const float s0 = simd_sum(sumf0[row]);
        const float s1 = simd_sum(sumf1[row]);
        const float s2 = simd_sum(sumf2[row]);
        const float s3 = simd_sum(sumf3[row]);
        const float s4 = simd_sum(sumf4[row]);
        const float s5 = simd_sum(sumf5[row]);
        const float sum_all = (((((s0 + s1) + s2) + s3) + s4) + s5);
        if (tiisg == 0) {
            dst_f32[first_row + row] = sum_all;
        }
    }
...
[[host_name("kernel_dsv4_ffn_moe_decode_stage_q2_K_f32")]]
kernel void kernel_dsv4_ffn_moe_decode_stage_q2_K_f32(
        constant ggml_metal_kargs_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    device float * dst_f32 = (device float *)(dst + (uint64_t) token * (uint64_t) args.ne1 * args.nb1);
    for (int row = 0; row < nr0 && first_row + row < args.ne0; row++) {
        const float s0 = simd_sum(sumf0[row]);
        const float s1 = simd_sum(sumf1[row]);
        const float s2 = simd_sum(sumf2[row]);
        ...
        const float s5 = simd_sum(sumf5[row]);
        if (tiisg == 0) {
            dst_f32[(uint64_t) 0 * args.nb1/sizeof(float) + first_row + row] = s0;
            dst_f32[(uint64_t) 1 * args.nb1/sizeof(float) + first_row + row] = s0 + s1;
            dst_f32[(uint64_t) 2 * args.nb1/sizeof(float) + first_row + row] = (s0 + s1) + s2;
            dst_f32[(uint64_t) 3 * args.nb1/sizeof(float) + first_row + row] = ((s0 + s1) + s2) + s3;
            dst_f32[(uint64_t) 4 * args.nb1/sizeof(float) + first_row + row] = (((s0 + s1) + s2) + s3) + s4;
            dst_f32[(uint64_t) 5 * args.nb1/sizeof(float) + first_row + row] = ((((s0 + s1) + s2) + s3) + s4) + s5;
        }
    }
...
[[host_name("kernel_mul_mv_id_sum6_weighted_q2_K_f32")]]
kernel void kernel_mul_mv_id_sum6_weighted_q2_K_f32(
        constant ggml_metal_kargs_mul_mv_id & args,
        constant ulong2 & weight_strides,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
        device const char * ids,
    ...
    device const int32_t * token_ids = (device const int32_t *)(ids + (uint64_t) token * args.nbi1);
    device const char * token_src1 = src1 + (uint64_t) token * args.nb12;
    device const char * token_weights = weights + (uint64_t) token * weight_strides.y;
    ...
    for (int expert_slot = 0; expert_slot < 6; expert_slot++) {
        const int32_t expert = token_ids[expert_slot];
        const float route_weight = *(device const float *)(token_weights + (uint64_t) expert_slot * weight_strides.x);
        device const block_q2_K * x = (device const block_q2_K *)(src0s + (uint64_t) expert*args.nb02 + (uint64_t) first_row*args.nb01);
        device const float * y = (device const float *)(token_src1 + (uint64_t) expert_slot*args.nb11);
        device const float * y4 = y + ix * QK_K + 128 * iq + 8 * ir;
        ...
        for (int ib = ix; ib < nb; ib += 4) {
            ...
            float4 sumy = {0.f, 0.f, 0.f, 0.f};
            for (short i = 0; i < 8; ++i) {
                yl[i+ 0] = route_weight * y4[i+ 0]; sumy[0] += yl[i+ 0];
                yl[i+ 8] = route_weight * y4[i+32]; sumy[1] += yl[i+ 8];
                yl[i+16] = route_weight * y4[i+64]; sumy[2] += yl[i+16];
                yl[i+24] = route_weight * y4[i+96]; sumy[3] += yl[i+24];
            }
...
[[host_name("kernel_dsv4_routed_moe_down_slots_q2_K_f32")]]
kernel void kernel_dsv4_routed_moe_down_slots_q2_K_f32(
        constant ggml_metal_kargs_mul_mv_id & args,
        constant ulong2 & weight_strides,
        device const char * src0s,
        device const char * src1,
    ...
    for (int row = 0; row < nr0 && first_row + row < args.ne0; row++) {
        ...
        const float s5 = simd_sum(sumf5[row]);
        if (tiisg == 0) {
            dst_f32[(uint64_t) 18 * args.nb1/sizeof(float) + first_row + row] = s0;
            dst_f32[(uint64_t) 19 * args.nb1/sizeof(float) + first_row + row] = s1;
            dst_f32[(uint64_t) 20 * args.nb1/sizeof(float) + first_row + row] = s2;
            dst_f32[(uint64_t) 21 * args.nb1/sizeof(float) + first_row + row] = s3;
            dst_f32[(uint64_t) 22 * args.nb1/sizeof(float) + first_row + row] = s4;
            dst_f32[(uint64_t) 23 * args.nb1/sizeof(float) + first_row + row] = s5;
            dst_f32[(uint64_t) 24 * args.nb1/sizeof(float) + first_row + row] = s0;
            dst_f32[(uint64_t) 25 * args.nb1/sizeof(float) + first_row + row] = s0 + s1;
            dst_f32[(uint64_t) 26 * args.nb1/sizeof(float) + first_row + row] = (s0 + s1) + s2;
            dst_f32[(uint64_t) 27 * args.nb1/sizeof(float) + first_row + row] = ((s0 + s1) + s2) + s3;
            dst_f32[(uint64_t) 28 * args.nb1/sizeof(float) + first_row + row] = (((s0 + s1) + s2) + s3) + s4;
            dst_f32[(uint64_t) 29 * args.nb1/sizeof(float) + first_row + row] = ((((s0 + s1) + s2) + s3) + s4) + s5;
        }
    }
