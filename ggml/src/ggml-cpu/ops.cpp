...
static float ggml_dsv4_fp4_quant_dequant(float x) {
    static constexpr float values[8] = { 0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f };

    const float sign = x < 0.0f ? -1.0f : 1.0f;
    const float ax = std::min(std::fabs(x), 6.0f);

    int best = 0;
    float best_diff = ax;
    for (int i = 1; i < 8; ++i) {
        const float diff = std::fabs(ax - values[i]);
        if (diff < best_diff || (diff == best_diff && (i & 1) == 0 && (best & 1) != 0)) {
            best = i;
            best_diff = diff;
        }
...
void ggml_compute_forward_dsv4_hadamard_fp4_quantize(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    GGML_ASSERT((src0->ne[0] & (src0->ne[0] - 1)) == 0);
    GGML_ASSERT(src0->ne[0] % 32 == 0);
    GGML_ASSERT(src0->ne[0] <= 256);

    const int64_t dim = src0->ne[0];
    const int64_t n_rows = src0->ne[1] * src0->ne[2] * src0->ne[3];
    const int64_t row_start = (n_rows * params->ith) / params->nth;
    const int64_t row_end   = (n_rows * (params->ith + 1)) / params->nth;
    const float inv_sqrt_dim = 1.0f / std::sqrt(float(dim));
    ...
    for (int64_t row = row_start; row < row_end; ++row) {
        ...
        for (int64_t i = 0; i < dim; ++i) {
            v[i] = *(const float *) (src_base + i*src0->nb[0]);
        }

        for (int64_t step = 1; step < dim; step <<= 1) {
            for (int64_t base = 0; base < dim; base += 2*step) {
        ...
        for (int64_t block = 0; block < dim; block += 32) {
            float amax = 0.0f;
            for (int64_t i = 0; i < 32; ++i) {
                v[block + i] *= inv_sqrt_dim;
                amax = std::max(amax, std::fabs(v[block + i]));
            }

            amax = std::max(amax, 6.0f * std::ldexp(1.0f, -126));
            const float scale = std::ldexp(1.0f, int(std::ceil(std::log2(amax / 6.0f))));
            for (int64_t i = 0; i < 32; ++i) {
                const float q = std::clamp(v[block + i] / scale, -6.0f, 6.0f);
